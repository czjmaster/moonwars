...
## 5-0. ZMIANY update66 (NAJNOWSZE — POLOWANIE NA MAPIE + CZTERY RACJE)
...
## 5-0a. ZMIANY update65 (CIAŁO TO DECYZJA, NIE SPRZĄTANIE)
...
## 5-0b. ZMIANY update64 (LISTA GOŃCZA)
...
## 5-0c. ZMIANY update63 (CELA, JENIEC I CIAŁO W WORKU)
...
## 5-0d. ZMIANY update62 (NIKT SIĘ NIE PODDAJE RZEŹNIKOWI)
...
## 5-0e. ZMIANY update61 (ŚWIAT CZYTA KARMĘ)
...
## 5-0f. ZMIANY update60 (DZIAŁO PAMIĘTA SWOJĄ KOMORĘ)
...
## 5-0g. ZMIANY update59 (DZIAŁO, KTÓRE NIE STRZELA, MÓWI DLACZEGO)
...
## 5-0h. ZMIANY update58 (He-3 WIDOCZNY, KSIĘŻYCE DO ODKRYCIA)
...
## 5-0i. ZMIANY update57 (ZAŁOGA NIE ZMIENIA KORPORACJI, CONTINUE W BAZIE)
...
## 5-0j. ZMIANY update56 (He-3, MOON GATE, REGIONY)
...
## 5-0k. ZMIANY update55 (DRUGA TURA POPRAWEK Z TESTÓW NA ŻYWO)
...
## 5-0l. ZMIANY update54 (15 POPRAWEK Z TESTÓW GRACZA NA ŻYWO)
...
## 5-0m. ZMIANY update53 (8 ROZKAZÓW SPECJALNYCH, WSZYSTKIE ROZKAZY W JEDNYM MIEJSCU)
...
## 5-0n. ZMIANY update52a (SPECJALIZACJE WYJAŚNIONE, TECZKA DOWÓDCY, UI MESY)
...
## 5-0o. ZMIANY update52 (25 RANG, DOWÓDCA 1–24, PLANSZA PO KWADRACIE, GRA PO ANGIELSKU)
...
## 5-0p. ZMIANY update51 (2× XP, ROZKAZY POD KAPITANEM, AWANS ZE SUFITEM)
...
## 5-0q. ZMIANY update50 (KARMA MA ŹRÓDŁA, KAPSUŁA LATA, WRÓG MA KAPITANA)
...
## 5-0r. ZMIANY update49a (STÓŁ TESTOWY DO PLANSZY CPU)
...
## 5-0s. ZMIANY update49 (PLANSZA CPU, CHIPY, KARMA JAKO GEOMETRIA)
...
## 5-0t. ZMIANY update48 (EKRAN ŁADOWNI: KLIK, PODZIAŁ STOSU, NIC NIE ZNIKA)
...
## 5-0u. ZMIANY update47 (SKAFANDRY, POWIETRZE, GŁÓD, KOTY)
...
## 5-0v. ZMIANY update46 (CARGO RETROFIT SKASOWANY)
...
## 5-0w. ZMIANY update45 (KOTY KSIĘŻYCOWE)
...
## 5-0x. ZMIANY update44 (raport gracza: exploit z osłonami, mesa jako budynek)
...
## 5-0y. ZMIANY update43 (KAPITAN, mesa, XP z konsoli, korporacje wrogów)
...
## 5-0z. ZMIANY update42 (raport z testów gracza: zaraza, ranni, dźwięk, abordaż)
...
## 5-0aa. ZMIANY update41 (JEDNA SIATKA KADŁUBOWA, kafle silnika i dziobu)
...
## 5-0ab. ZMIANY update40 (AUDYT: ekran opcji, martwe mechaniki, wycieki stanu, UI)
...
## 5-0ac. ZMIANY update39 (He2 jako ładunek, mgła na mapie, szczury księżycowe, HP w koszarach)
...
## 5-0ad. ZMIANY update38 (zapis postępu sektora, sloty w modułach, combat tylko wręcz, jaja w różnych pokojach)
...
## 5-0ae. ZMIANY update37 (chodzenie po podłodze, wraki z powietrzem, ukryte jaja, mniej chromu)
...
## 5-0af. ZMIANY update36 (hakowanie drzwi, sporne moduły, cmentarz z historią służby, kontrakt startowy)
...
## 5-0ag. ZMIANY update35 (JEDEN MAGAZYN, klasy broni, cmentarz, 6 bugów załogi)
...
## 5-0ah. ZMIANY update34 (WAREHOUSE wchłonięty przez SUPPLY, UI bazy, załoga przy konsolach, 3 realne bugi)
...
## 5-0ai. ZMIANY update33 (magazyn bazy jako prawdziwa siatka)
...
## 5-0aj. ZMIANY update32 (kolory ładowania, reaktor jako moduł, przebudowa UI bazy)
...
## 5-0ak. ZMIANY update31 (jaja we wrakach, sprite'y pająków, grafika broni, nazwy egipskie)
...
## 5-0al. ZMIANY update30 (bilans modułów, drzwi na czas, grafika broni, naprawa w bazie)
...
## 5-0am. ZMIANY update29 (broń tylko w mount albo w skrzyni, kolory korporacji, martwe wraki, hangar)
...
## 5-0an. ZMIANY update28 (dokowanie, wraki po których się chodzi, pająki i wirus)
...
## 5-0ao. ZMIANY update27 (łączenie stosów, skrytka na broń, winda po abordażu)
...
## 5-0ap. ZMIANY update26 (STOSY: ilość JEST przedmiotem)
...
## 5-0aq. ZMIANY update25 (amunicja i broń w ładowni, salwy, więcej wraków)
...
## 5-0ar. ZMIANY update24 (ładownia siatkowa + ekran łupu)
...
## 5-0as. ZMIANY update23 (UI portów + oprawa graficzna)
...
## 5-0au. ZMIANY update21 (hotfix + nowy typ testów)
...
## 5-0av. ZMIANY update20 (DUŻA: meta-progresja)
...
|---|---|---|
| **RELIGIE** + święta, bagaż 2 sloty, kot 1 | `projekt-religie.md` | następna w kolejce — racje gotowe, `willEat` czeka na reguły |
| **RATOWANIE WIĘŹNIÓW** z wrogich statków | `projekt-jeniec-cela.md` §2.5 | cela gotowa, jest gdzie ich sadzać |
| **CELE DODATKOWE** (licznik + warunek + nagroda, linia na HUD) | `projekt-lista-goncza.md` §2 | — |
| **KONTRAKTY NA KRAŃCACH KARMY** (wariant A) | `projekt-karma-dowodcy.md` §3.2 pkt 4 | zatwierdzony wariant A: **dwa NOWE zlecenia**, nie blokowanie trzech istniejących |
| **GRAFIKA** | `brief-graficzny-ui.md` | patrz 6.4 |

**Kontrakty — dlaczego wariant A.** W grze są trzy kontrakty i są drabinką
trudności (1/2/3 sektory), nie menu smaków. Zablokowanie któregoś za progiem
karmy **odbiera grę** zamiast dodawać wybór. Więc: trzy istniejące zostają
zawsze dostępne, dochodzą dwa nowe — jedno tylko przy wysokiej karmie
(ratunkowe: mniej CC, płaci karmą i chipem), jedno tylko przy niskiej (robota,
której nikt uczciwy nie weźmie: więcej CC, kosztuje karmę). Progi czytać z
`Chips.wallColumn`, **nie wymyślać nowych liczb**. Kontrakt, którego nie ma,
**musi powiedzieć dlaczego** — zasada zamkniętego doku z update61.

### 6.2. Długi z ostatnich paczek

- **Uciekinier z celi nie wraca na listę gończą.** Teraz jest już i gdzie go
  wpisać, i gdzie go potem znaleźć (węzeł na mapie). Wszystko już jest
  (`prisoners[].wantedId`, `Save.addWanted`). Brakuje decyzji gracza: wraca z tą
  samą ceną czy z wyższą, bo raz już uciekł.
- **Uciekinier nie chodzi po korytarzu** — znika za ekranem. Wersja z chodzącym
  wymaga `isPrisoner` w kilkunastu filtrach obok `isVermin`/`isSpider`/`isBeast`
  — ta sama klasa błędu co kot w baraku (update45). Porządnie albo wcale.
- **Jeniec nie je** — czeka na paczkę racji.
- **`VENT` kosztuje −3 karmy od update65** — przy dodawaniu religii pamiętać, że
  część wyznań ma to jako obrazę, a część nie; wtedy ta jedna liczba przestaje
  wystarczać (decyzja gracza z 2026-09-08).
- **Cela pożycza ikonę** `icon_medbay`, cloak i repair bay też pożyczają cudze.

### 6.3. Otwarte bugi i pytania balansowe

- **NIEODTWORZONE: moduł medyka niewidoczny w hangarze.** Szukane przez trzy
  paczki, prześledzone przez `addModule → serialise → deserialise → _levels` i
  pełny obieg `launch → returnFromRun`. Potrzebne: kadłub, miejsce zakupu, czy
  hangar był pełny.
- **Hull Cannon** — 3 mocy, 1 rakieta na strzał, 18 s ładowania, rakieta 6 CC.
  Cztery strzały i zapas znika. Komunikaty ostrzegają od update59, ale czy to
  działo w ogóle się opłaca, to osobne pytanie.
