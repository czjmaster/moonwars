...
const Captain = (() => {
   ...
   * Returns zeroes for anybody of another corporation, for beasts, and
   * whenever no captain is flying — so every caller can just multiply.
   */
  function bonusFor(crew) {
    ...
    if (crew.race !== _active.race) return empty;
    const per = CAPTAIN_CORP_BONUS[_active.race];
    if (!per) return empty;
    const lvl = Utils.clamp(_active.level, 0, CAPTAIN_MAX_LEVEL);
    return {
      hp:     (per.hp     ?? 0) * lvl,
      speed:  (per.speed  ?? 0) * lvl,
      repair: (per.repair ?? 0) * lvl,
      melee:  (per.melee  ?? 0) * lvl,
    };
  }
  ...
  return {
    ...
    bonusFor, bonusLines, reseatMaxHp,
