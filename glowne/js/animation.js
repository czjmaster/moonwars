...
const Animation = (() => {
  ...
  /* ── THE HELMET (update47) ────────────────────────────────
   *
   * Everybody aboard is in vacuum gear now — they have to be, because
   * update47 gives every one of them a bottled air supply that runs
   * out. A suit you cannot see is a rule the player has to be told
   * in text; a glass bubble over the head is one he reads off the
   * sprite.
   *
   * Drawn LAST, over the head, so the face and the visor still show
   * through it. ONE function, called from every crew state and from
   * the cat — draw the bubble in six places and six bubbles will
   * drift apart, which is exactly how the visor colours ended up
   * different in three of these already.
   */
  function _helmet(ctx, cx, cy, r) {
    ctx.save();
    // The glass itself: barely there, so it never hides the man.
    ctx.fillStyle = 'rgba(180,220,255,0.16)';
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill();
    ctx.strokeStyle = 'rgba(200,232,255,0.85)';
    ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.stroke();
    // Specular arc, top-left — the one thing that makes it read as glass.
    ctx.strokeStyle = 'rgba(255,255,255,0.75)';
    ctx.lineWidth = 1.4;
    ctx.beginPath();
    ctx.arc(cx, cy, r - 2.2, Math.PI * 1.05, Math.PI * 1.55);
    ctx.stroke();
    // Neck ring: where the bubble bolts to the suit.
    ctx.strokeStyle = 'rgba(159,180,204,0.9)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(cx - r * 0.55, cy + r * 0.86);
  ...
  /* THE MOON CAT (update45).
   *
   * Deliberately NOT a recoloured rat. The rat is long, low and flat to
   * the deck because it reads as vermin; the cat sits UP on its
   * haunches when idle, has a tall upright tail and pointed triangular
   * ears, so that at sprite size the two silhouettes can never be
   * mistaken for one another — which matters, because one of them you
   * want aboard and the other you do not. */
  function _genCat(baseColor, mode = 'idle') {
    return _makeFrames(6, (ctx, t) => {
      const step = Math.sin(t * Math.PI * 2);
      const up   = mode === 'idle';          // sitting, tail curled
      ctx.save();
      ctx.translate(32, 40 + (up ? step * 0.4 : 0));

      /* THE CAT USED TO WALK BACKWARDS ONE WAY (update47).
         CrewMember.draw mirrors the sprite when _facing is -1, and
         _facing 1 means "heading right" — but this art was drawn with
         the head at negative x, i.e. facing LEFT. So the unmirrored
         frame, which is the one used walking RIGHT, showed a cat
         moonwalking tail-first, and only the mirrored one looked
         right. Draw the art facing RIGHT and both directions read
         correctly; the flip in draw() stays the single place that
         knows about direction. */
      ctx.scale(-1, 1);

      if (mode === 'fight') ctx.rotate(-0.2 * step);

      // Tail: upright and curling when sitting, streaming out when moving.
      ctx.strokeStyle = baseColor;
      ctx.lineWidth = 2.4;
      ctx.beginPath();
      ctx.moveTo(7, up ? 1 : 0);
      if (up) ctx.quadraticCurveTo(15, -6 + step, 12 + step, -13 - step);
      else    ctx.quadraticCurveTo(16 + step * 2, -4 + step * 2, 21 + step * 3, -7 - step * 2);
      ctx.stroke();

      if (up) {
        // Sitting: haunches on the deck, chest lifted.
        ctx.fillStyle = baseColor;
        ctx.beginPath(); ctx.ellipse(2, 1, 7, 6, 0, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#07080f'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.ellipse(2, 1, 7, 6, 0, 0, Math.PI * 2); ctx.stroke();
        ctx.fillStyle = baseColor;
        ctx.beginPath(); ctx.ellipse(-3, -5, 5, 6, 0, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#07080f';
        ctx.beginPath(); ctx.ellipse(-3, -5, 5, 6, 0, 0, Math.PI * 2); ctx.stroke();
      } else {
        // Prowling: four legs and a level back.
        ctx.strokeStyle = '#0f0d0a';
        ctx.lineWidth = 2;
        [[-6, 0], [-2, Math.PI], [3, Math.PI / 2], [6, -Math.PI / 2]].forEach(([lx, ph]) => {
          const sw = (mode === 'fight' ? 3.4 : 3.0) * Math.sin(t * Math.PI * 2 + ph);
          ctx.beginPath(); ctx.moveTo(lx, 2); ctx.lineTo(lx + sw * 0.6, 9); ctx.stroke();
        });
        ctx.fillStyle = baseColor;
        ctx.beginPath(); ctx.ellipse(0, -1, 10, 5, 0, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#07080f'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.ellipse(0, -1, 10, 5, 0, 0, Math.PI * 2); ctx.stroke();
      }

      // Head, high and round — never the rat's pointed snout.
      const hx = up ? -5 : -11, hy = up ? -12 : -4;
      ctx.fillStyle = baseColor;
      ctx.beginPath(); ctx.arc(hx, hy, 5, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = '#07080f'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(hx, hy, 5, 0, Math.PI * 2); ctx.stroke();

      // TWO pointed ears — the whole silhouette hangs off these.
      ctx.fillStyle = baseColor;
      [[-3.2, -1.2], [1.2, -3.4]].forEach(([ex, tipx]) => {
        ctx.beginPath();
        ctx.moveTo(hx + ex, hy - 3);
        ctx.lineTo(hx + tipx, hy - 9);
        ctx.lineTo(hx + ex + 3.4, hy - 3.6);
        ctx.closePath(); ctx.fill();
      });

      // Eyes: a slit that catches the light, and blinks when idle.
      const blink = up && Math.sin(t * Math.PI * 2) > 0.93;
      ctx.fillStyle = blink ? '#07080f' : '#9dff6a';
      ctx.beginPath(); ctx.ellipse(hx - 2, hy - 0.5, 1.3, blink ? 0.4 : 1.6, 0, 0, Math.PI * 2); ctx.fill();
      ctx.beginPath(); ctx.ellipse(hx + 2.4, hy - 0.5, 1.3, blink ? 0.4 : 1.6, 0, 0, Math.PI * 2); ctx.fill();

      /* The cat gets a bubble too. It has a tank of its own in
         update47 and it walks through vented compartments the crew
         will not — so it had better be wearing something. Wide
         enough to clear the ears. */
      _helmet(ctx, hx, hy - 1.5, 8.5);
  ...
  function catAnim(state = 'idle', color = '#3a3a42') {
    const key = state + '_' + color;
    if (!_catCache.has(key)) _catCache.set(key, _genCat(color, state));
    return new AnimationInstance(_catCache.get(key),
  ...
  return {
    init,
    crewIdle, crewWalk, crewRepair, crewFight, crewDie, crewByColor, ratAnim, catAnim,
