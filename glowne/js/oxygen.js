...
class RoomOxygen {
  ...
  update(dt, o2Power, breachCount = 0, isVacuum = false, crew = []) {
    ...
    /* ── BOTTLED AIR (update47) ────────────────────────────
       ...
       not breathe at all (SUIT_AIR.NO_LUNGS), so the hatch is no
       longer the universal answer to everything living in your vents.
       Fire and the cat are what is left for a spider. */
    const air = (typeof SUIT_AIR !== 'undefined') ? SUIT_AIR : null;
    if (air) {
      const breathable = this.level > 0;
      crew.forEach(c => {
        if (!c || c.dying || c.dead) return;
        // Asked BEFORE the tank is touched, so a creature with no
        // lungs never carries an air value to go stale in the save.
        if (c.breathes && !c.breathes()) return;
