...
const CARGO_ITEMS = {
  ...
  /* ── He-3, THE MINERAL (update56) ─────────────────────────
   *
   * NOT fuel, and the distinction matters more than it looks. The ship
   * runs on He2 — `kind: 'fuel'`, counted by `Ship.fuelCount()`, burnt
   * by every jump. He-3 is a strategic MINERAL: it is mined, carried,
   * bought and sold, and in this demo it does nothing else at all.
   *
   * It is deliberately `kind: 'trade'`. Anything else and the fuel
   * counter would pick it up, and the first time a player jumped on a
   * hold full of He-3 we would have two different substances sharing
   * one number — the exact failure this project keeps deleting.
   *
   * `tag: 'he3'` is how the Moon Gate parts list will count it later
   * without knowing anything about cargo internals.
   */
  he3_ore: {
    label: 'He-3 Ore', short: 'He3',
    w: 1, h: 1, value: 30, col: '#cfe4ff', kind: 'trade', tag: 'he3',
    stackMax: 10, unitValue: 30,
    desc: 'Helium-3, scraped out of lunar regolith. The Gate runs on it '
        + 'and nothing else does — not yet. Worth carrying home.',
  },
  /* A BODY IN A BAG (update63). Two cells, because that is what it
     costs to carry a man home instead of ore — the whole point of the
     dilemma at the end of a run. Sealed, so unlike a corpse left on
     the deck it does not rot, does not spread the plague and does not
     bring rats: the player answered that question directly. */
  body_bag: {
    label: 'Body Bag', short: 'BODY',
    w: 2, h: 1, value: 60, col: '#8a94a8', kind: 'trade', tag: 'body',
    desc: 'A sealed bag with a name on the tag. The bounty office pays '
        + 'for it — half of what they pay for the man alive.',
  },
  /* A CAT IS ONE CELL (update65). Same tag as the body bag above, so
     the dock has ONE rule for "a bag with somebody in it" — only the
     shape on the grid differs, which is the only thing that really
     does differ. */
  pet_bag: {
    label: 'Pet Bag', short: 'PET',
    w: 1, h: 1, value: 30, col: '#8a94a8', kind: 'trade', tag: 'body',
    desc: 'A small sealed bag. He was crew too.',
  },
  medkit: {
    label: 'Medical Supplies', short: 'MED',
    w: 1, h: 1, col: '#1aff8c', kind: 'heal',
    stackMax: 10, unitValue: 6, healPerDose: 25,
    desc: 'Up to 10 doses. USE one to cure the corpse plague — or, with '
        + 'nobody ill, to patch up your worst-hurt crewman. It does '
        + 'nothing for a void-spider bite; only a station clinic can.',
  },
  ...
  /* RATIONS BECAME A STACK (update47).
     They used to be a `trade` parcel — something you sold, one cell
     for one lump sum, where the tag 'food' meant nothing except that
...
/* ── CPU CHIPS ARE ORDINARY CARGO (update49) ─────────────────
 *
 * Twelve chips at four levels is forty-eight catalogue entries, and
 * writing them out by hand would be forty-eight chances to get a
 * width wrong. They are GENERATED from the one table in chips.js, so
 * a chip's size, family and colour have exactly one source.
 *
 * The point of making them cargo at all is the project's hardest
 * invariant: a chip is on the shelf, or in the hold, or on a
 * commander's board — never in two of them. The alternative, a separate
 * list of "installed upgrades", is what gave update35 three
 * warehouses for one shelf.
 *
 * Price by level: a level I chip is worth about a recruit, a IV about
 * a small hull. Level IV never appears in a shop — the spec puts it
 * behind Apophis — so its price is what a port pays you for one.
 */
const CHIP_VALUE = [40, 85, 170, 340];

/* Called from BOTH files, so the pair loads in either order: cargo.js
   runs it if chips.js got in first, and chips.js runs it if it did
   not. Writing the same entries twice is harmless — they are built
   from the one table and are identical. A stale index.html that has
   cargo.js but not chips.js is exactly the case LATE_MODULES exists
   for, and this is what keeps the catalogue whole when it fires. */
function registerChipItems() {
  if (typeof CHIP_DEFS === 'undefined') return false;
  Object.entries(CHIP_DEFS).forEach(([key, def]) => {
    for (let lvl = 1; lvl <= 4; lvl++) {
      const shape = chipShape(def.family, lvl);
      const roman = CHIP_LEVEL_LABELS[lvl - 1];
      CARGO_ITEMS[chipItemKey(key, lvl)] = {
        label: `${def.label} ${roman}`,
        short: 'CHIP',
        w: shape.w, h: shape.h,
        col: CHIP_FAMILIES[def.family].col,
        kind: 'chip',
        value: CHIP_VALUE[lvl - 1],
        chipKey: key, chipLevel: lvl, chipFamily: def.family,
        desc: `${def.desc} Mounted only at base, on the commander's CPU board.`,
      };
    }
  });
  return true;
...
function cargoCrateForWeapon(defKey) {
  ...
  /* 65, NOT 75 (update75, player's call). At 75 the split was 2/4/2 and
     the HEAVY Laser (70 CC) came in a medium crate, so the gun's name
     and the box's name disagreed on the same screen. At 65 it is 2/3/3
     and "Heavy" means heavy again.

     Still ONE number, deliberately: the alternative was a size field on
     every gun in WEAPON_DEFS, a second register beside the price, and
     two copies of one number in this project always end the same way. */
  if (cost <= 50) return 'gun_crate_s';
  if (cost <= 65) return 'gun_crate';
  return 'gun_crate_l';
}
...
class CargoItem {
  ...
  value(portType = 'general') {
    ...
    /* A BODY IS WORTH THE MAN IN IT (update63). The bounty travels on
       the bag, not in a second table beside it: the event that filled
       the bag quoted a figure, and this is that same figure rather
       than a lookup that could be balanced apart from it. */
    if (this.def.tag === 'body' && this.meta && this.meta.bounty > 0) v = this.meta.bounty;
    /* ONE OF YOUR OWN IS NOT MERCHANDISE (update65). A bag carrying a
       crewman home for burial has no price at a port — the reward for
       it is karma at the dock, and putting a number on him here would
       have let the player sell his own dead to a passing trader.
       Returned EARLY, not zeroed: the `Math.max(1, …)` at the bottom of
       this function exists so nothing tradeable is ever free, and it
       would have quietly priced a dead crewman at 1 CC. */
    if (this.meta && this.meta.crewBody) return 0;
...
class CargoGrid {
   ...
   *
   * A grid may carry an optional `rule` (update49, for the CPU board):
   *   rule.cell(x, y)              — is this CELL usable at all?
   *   rule.item(item, x, y, w, h)  — may THIS item stand at this spot?
   * The plain hold and the base shelf have no rule and behave exactly
   * as before. The rule is asked, never cached: the board's blocked
   * column moves with the commander's karma, and a remembered answer
   * would be a second copy of that karma.
   */
  fits(item, gx, gy, ignore = null) {
    ...
    for (let y = 0; y < m.length; y++) {
      for (let x = 0; x < m[y].length; x++) {
        ...
        if (this.rule?.cell && !this.rule.cell(cx, cy)) return false;
      }
    }
    if (this.rule?.item &&
        !this.rule.item(item, gx, gy, m[0].length, m.length)) return false;
    return true;
  }

  /** Is this cell off limits entirely? Drawn as a wall, never filled. */
  blockedAt(x, y) { return !!(this.rule?.cell) && !this.rule.cell(x, y); }
   ...
   *  A grid marked `noRotate` (the CPU board) is tried as-is: the spec
   *  forbids turning a chip once it is on the board, and an autoPlace
   *  that quietly rotated it would be a back door into exactly that. */
  autoPlace(item) {
    const startRot = item.rot;
    const turns = this.noRotate ? 1 : 4;
  ...
  /**
   * COUNT BY TAG (update56).
   *
   * `countOf` answers by KIND, which is what the fuel and missile
   * readouts need. He-3 is `kind: 'trade'` like a dozen other things —
   * it is a mineral, not a tank — so counting it needs the tag instead.
   * The Moon Gate's parts list reads the shelf the same way, which is
   * why this lives on the grid rather than in either caller.
   *
   * Same rule as `countOf` about damaged stacks: a cracked crate is not
   * inventory you can spend.
   */
  countOfTag(tag) {
    return this.items.reduce(
      (n, it) => n + (it.def.tag === tag && !it.damaged
...
function cargoRollTable(sector = 1) {
  const t = [
    ...
    /* He-3 is the reason anybody flies out here (update56). Uncommon,
       and commoner deeper in — but never so common that a wreck becomes
       a Gate delivery. It has no use in the demo; it is worth carrying
       because it SELLS, and because the Gate window says what it is
       for. */
    ['he3_ore',        2 + Math.floor(sector / 2)],
  ];
  return t;
}
...
function makeWreckGrid(sector = 1, opts = {}) {
  ...
  /* A CHIP IN A WRECK (update49). Uncommon on purpose: a chip is the
...
if (typeof window !== 'undefined') {
  ...
  window.registerChipItems = registerChipItems;
