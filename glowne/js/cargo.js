 ...
 * captain's board — never in two of them. The alternative, a separate
  ...
  Object.entries(CHIP_DEFS).forEach(([key, def]) => {
    for (let lvl = 1; lvl <= 4; lvl++) {
      ...
      CARGO_ITEMS[chipItemKey(key, lvl)] = {
        ...
        desc: `${def.desc} Montaż wyłącznie w bazie, na planszy CPU kapitana.`,
      };
    }
  });
  return true;
...
class CargoGrid {
   ...
   * column moves with the captain's karma, and a remembered answer
...
function makeWreckGrid(sector = 1, opts = {}) {
  ...
  /* A CHIP IN A WRECK (update49). Uncommon on purpose: a chip is the
     one piece of salvage that changes a captain permanently, and a
