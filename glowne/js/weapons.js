...
class Projectile {
  ...
  /**
   * A BEAM IS A LINE FROM THE BARREL TO THE CUT (update80).
   *
   * It used to be a horizontal bar drawn ACROSS the target at `this.y`
   * and starting at the target's own left edge — a beam with no
   * source, which was invisible as a flaw only because every shot in
   * the game left from the same point on the flank anyway. Now that a
   * bolt leaves its own gun, a beam that appears out of empty space
   * beside the enemy is the one weapon that still looks broken.
   *
   * So it is drawn from where it was fired — `this.x, this.y`, which a
   * beam never moves from — to the point its cut has reached across
   * the target hull. The line pivots as the cut travels, which is what
   * a beam slicing along a hull should look like, and it is anchored
   * to the muzzle at the near end.
   */
  drawBeam(ctx, shipX, shipY, shipW) {
    if (this.type !== 'beam' || this.beamProgress <= 0) return;
    const cutX = shipX + shipW * this.beamProgress;
    const cutY = shipY;
    const grad = ctx.createLinearGradient(this.x, this.y, cutX, cutY);
    grad.addColorStop(0, 'rgba(255,200,50,0.85)');
    grad.addColorStop(0.65, 'rgba(255,100,20,0.9)');
    grad.addColorStop(1, 'rgba(255,50,0,0.35)');
...
class Weapon {
  ...
  constructor(defKey, slot = 0) {
    ...
    this.slot      = slot;
    /* THE BAY THIS GUN IS BOLTED INTO (update60). `slot` is its place in
       the ship's weapon ARRAY; `roomId` is the module it draws power and
       crew from. They used to be the same question answered by position,
       which meant adding a module could re-point every gun aboard.
       Null until a Ship installs it — see Ship.weaponRoomFor. */
    this.roomId    = null;
    this.charge    = 0;   // 0–1
    this.armed     = false;
