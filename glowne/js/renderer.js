...
const Renderer = (() => {
   ...
   */
  /* 150 wide, not 96. At 96 the reason on a refused row was clipped to
     "no ro…", which is a grey box with noise in it rather than an
  ...
  function drawBodyMenu(ctx, x, y, name, refusal, acts) {
    ...
    R.items.forEach(it => {
      ...
      ctx.fillStyle = col;
      ctx.font = '10px Share Tech Mono, monospace';
      ctx.fillText(LABEL[it.act], it.x + 6, it.y + 12);
      if (why) {
        /* THE REASON, NOT JUST A GREY BOX. Same rule as the shut dock
           in update61: a refusal nobody explains reads as a bug. */
        ctx.fillStyle = '#5f7893';
        ctx.font = '7px Share Tech Mono, monospace';
        ctx.fillText(_clipTo(ctx, why, it.w - 46), it.x + 42, it.y + 12);
      }
    });
    return R;
  }
