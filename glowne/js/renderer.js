...
const Renderer = (() => {
  ...
  function drawMapScreen(sectorMap, hoverNodeId = null) {
    ...
    if (run) {
      ...
      ctx.textAlign = 'right';
      ctx.fillStyle = run.fuel <= 2 ? '#ff2d44' : '#ff5566';
      ctx.fillText(`He2 ${run.fuel}`, ox + 450, oy - 18);
      ctx.fillStyle = '#1aff8c';
  ...
  function drawShipThumb(ctx, layoutKey, x, y, w, h, opts = {}) {
    ...
    rooms.forEach(r => {
      ...
      if (rw > 22 && rh > 14) {
        ctx.fillStyle = bare ? '#5a6a86' : col;
        ctx.font = `${Math.max(7, Math.min(9, rw / 4))}px Share Tech Mono, monospace`;
        ctx.textAlign = 'center';
        ctx.fillText(SHORT[type] ?? '?', rx + rw / 2, ry + rh / 2 + 3);
      }
    });
