...
const SYSTEM_DEFS = {
  ...
  cloaking: {
    ...
    maxLevel: 2,
    description: 'Phase distortion field: +8% evasion per powered level.',
  },
  autorepair: {
...
class ShipSystem {
  ...
  effectivePower() {
    ...
    // UNCAPPED: the bonus can exceed the module level, so it works
    // even on a level-1 module with 1 allocated power.
    // …and AUTOMATIC: the +1 follows the Terra member into whatever
    // module they stand in — even one with ZERO allocated power.
    if (this.workingLevels > 0 &&
        this.crew.some(c => c && c.alive !== false && !c.dead && c.cyborg)) {
      p = p + 1;
    }
    return p;
  }
