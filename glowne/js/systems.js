...
const SYSTEM_DEFS = {
  ...
  /* THE BRIG (update63). Levels are CELLS, one prisoner each — which
     ...
     a man can work on. */
  brig: {
    label: 'Brig', icon: 'icon_medbay',
    maxLevel: 3,
    description: 'Holds prisoners — one cell per level. Cut its power and they start working the lock.',
  },
  medbay: {
...
class ShipSystem {
  ...
  /* THE POWER PIPS USED TO BE HERE.
    ...
    /* ── THE NAME GOES UNDER THE CEILING (update73) ─────────
     *
     * It used to sit on the bottom edge of the module, which was
     * clear air while a compartment was 72px tall. The room is 60
     * now, of which 10 is duct, so the crew's feet land at the top of
     * the name plate and a man stands in the middle of the word —
     * "We▮pons" on the first screenshot of this package.
     *
     * Up here it is out of everybody's way: the badge is in the left
     * corner, the module icon is in the middle, the crew are on the
     * deck below. Nothing else is drawn along this line.
     *
     * Found by LOOKING. No assertion in this suite measures a label
     * against a pair of boots. */
    ctx.save();
    ctx.font = '9px Share Tech Mono, monospace';
    ctx.fillStyle = powered ? 'rgba(200,232,255,0.85)' : 'rgba(192,144,144,0.85)';
    /* LEFT, not centred. The three standing spots are cx-26, cx and
       cx+26, so the middle of the module is precisely where a man's
       name tag will be — centring the sign here would put it under a
       crew tag in every compartment anybody is working in. */
    ctx.textAlign = 'left';
    ctx.fillText(this.label, x + 5, y - HULL_GRID.VENT_H + 8);
    ctx.restore();
  }
}
