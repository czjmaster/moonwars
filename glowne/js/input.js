...
const Input = (() => {
  ...
  const mouse = {
    ...
    leftPressed: false,
    leftReleased: false,
    /* A RIGHT CLICK IS AN EDGE, LIKE THE LEFT ONE (update65). It used
       to be a level only (`rightDown`), which nothing could act on
       once per press — and a context menu that opens on every frame
       the button is held is a menu that re-opens the moment you try
       to click a row in it. */
    rightPressed: false,
    scrollDelta: 0,
  };
  ...
  let _pendingRightPress = false;
  let _pendingScroll  = 0;
  const _pendingKeys     = new Set();
  const _pendingReleases = new Set();
  /* Characters typed since the last frame — see onKeyDown. */
  let _typed = [];
  let _typedFrame = [];
  ...
  function beginFrame() {
    ...
    mouse.rightPressed = _pendingRightPress;
    mouse.scrollDelta  = _pendingScroll;
    _pendingPress   = false;
    _pendingRelease = false;
    _pendingRightPress = false;
    _pendingScroll  = 0;
  }
  ...
  function onKeyDown(e) {
    ...
    keys.add(e.code);
    /* WHAT THE PLAYER ACTUALLY TYPED (update54).
       `e.code` is a key POSITION — KeyA is KeyA on a French keyboard
       too — which is right for the ship's hotkeys and useless for
       typing a name. `e.key` is the character, and it is collected
       here only as a per-frame queue: the rename field drains it, and
       anything nobody reads is dropped at the next beginFrame. No
       screen has to opt out of hotkeys, because nothing else looks at
       this list. */
    if (e.key && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
      if (_typed.length < 64) _typed.push(e.key);
    }
  }
  ...
  function onMouseDown(e) {
    ...
    if (e.button === 2) { mouse.rightDown = true; _pendingRightPress = true; }
  }
  ...
  return {
    ...
    isPressed,
    isReleased,
    /** Characters typed during the last frame, in order. */
    typedChars: () => _typedFrame.slice(),
    mouse,
    onCanvasClick,
