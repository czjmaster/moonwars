...
/* update51: EVERY rate DOUBLED. The measurements above stand — they
   ...
   which job pays best has not moved. Halve the fights-to-master
   numbers in the comment above to read the new pace. */
const XP_RATES = {
  weapons:   2.0,    // per shot from a manned gun      (update51: was 1.0)
  piloting:  16,     // per dodge, pilot at the helm    (update51: was 8)
  engines:   16,     // per dodge, hand at the drive    (update51: was 8)
  shields:   16,     // per recharged layer             (update51: was 8)
  repair:    0.5,    // PER SECOND of repairing         (update51: was 0.25)
  firefight: 10.0,   // PER SECOND of firefighting      (update51: was 5.0)
  breach:    12.0,   // PER SECOND of patching          (update51: was 6.0)
  combat:    20,     // per melee swing                 (update51: was 10)
};
...
const HUNGER = {
  ...
  FOOD: {
    ...
    /* `ration` IS GONE (update66) — a ration's worth lives on the cargo
       item now, because there are four of them. What is left here is
       the food that is NOT cargo: a rat and a spider egg. */
  },
};
...
class CrewMember {
  /* How far below his standing position a downed man is drawn, so a
     body and the hand working over it are two separate things to look
     at and to click. Read by the drawing here and by the body hit test
  ...
  constructor(cfg = {}) {
    ...
    /* HIS PLACE IN THE BARRACKS LIST (update57). The base issues this
       ticket once, when he first signs on; it rides along with him on a
       contract for exactly the same reason `id` does — he has to come
       home as the same man, in the same place on the roster. Undefined
       for anyone who was never in a barracks (enemies, station
       recruits, the greenhorns the guild sends), and the base stamps
       one the moment they walk in. */
    this.joined   = cfg.joined;
    ...
    /* IN A SLAB (update77). Restored from the save, so a reload finds
       him exactly where the freeze left him — and, with him, the two
    ...
    /* HIS FIGURE WITHOUT A COMMANDER'S BONUS ON IT (update68). Carried
       through the save so `reseatMaxHp` recomputes from it instead of
       stacking on top of last run's result — see serialise below. A
       record written before this existed simply has none, and the man
       is taken as he stands, which is what he was. */
    this.baseMaxHp = cfg.baseMaxHp ?? this.maxHp;
  ...
  strike(target, dmg) {
    ...
    target.takeDamage(dmg * (1 - resist), 'crew');
  ...
  draw(ctx) {
    ...
    /* ── A MAN IN A SLAB LOOKS LIKE ONE (update77) ──────────
     *
     * The roster mark says he is frozen; the DECK said nothing at all,
     * so a player looking at his ship saw three men standing in a room
     * and two of them could not do a thing. A screenshot caught that,
     * because no assertion can: the strip and the deck were both
     * "correct" and the ship still lied.
     *
     * Drawn as a slab AROUND him rather than a tint ON him: a tint
     * reads as damage or as an enemy, and this man is neither — he is
     * fine, he is just outside time. */
    if (this.frozen && !this.dead) {
      /* Geometry taken from the STANDING draw below — `anim.draw` is
         given his centre, so the slab is built round the same point.
         Writing the numbers out again is how a slab ends up floating
         over a man's head, which is what the first version of this did
         and what the screenshot showed. */
      const W = 20, H = 34, top = this.y - 17;
      ctx.save();
      ctx.globalAlpha = 0.5;               // him, behind the ice
      ...
      ctx.lineWidth = 1;
      ctx.beginPath(); ctx.roundRect(this.x - W / 2, top, W, H, 3);
      ctx.fill(); ctx.stroke();
      /* Frost across the face, so it reads as cold at a glance rather
         than as a selection box. */
      ctx.strokeStyle = 'rgba(200,240,255,0.5)';
      ctx.beginPath();
      ctx.moveTo(this.x - W / 2 + 3, top + 10);
      ctx.lineTo(this.x + W / 2 - 3, top + 16);
      ctx.moveTo(this.x - W / 2 + 3, top + 24);
      ctx.lineTo(this.x + W / 2 - 3, top + 18);
      ctx.stroke();
      ctx.restore();

      /* His name, and nothing else above him: no health bar, no
         markers. Nothing is happening to him — that is the point —
         but the player still has to know which of them is in there. */
      ctx.font = '9px Share Tech Mono, monospace';
      ctx.textAlign = 'center';
      const lw = ctx.measureText(this.name).width + 6;
      ctx.fillStyle = 'rgba(7,8,15,0.8)';
      ctx.fillRect(this.x - lw / 2, top - 13, lw, 11);
      ctx.fillStyle = '#7fd4ff';
      ctx.fillText(this.name, this.x, top - 4);
      return;
    }
    ...
    if (this.down) {
      /* ── A BODY LIES LOW IN THE ROOM (update68) ──────────────
       *
       * A man on the floor was drawn at the same height as a man
       * standing over him, so the two overlapped into one shape and
       * the player could not tell whom he was right-clicking. His
       * words: "załogant jak i ciało leżą w tym samym miejscu, więc
       * przenieś ciała trochę niżej".
       *
       * `BODY_DROP` is the offset, used by the DRAWING and by the hit
       * test in game.js alike — put it in one place and the thing you
       * can click stays exactly the thing you can see. */
      ctx.save();
      ctx.translate(this.x, this.y + 8 + CrewMember.BODY_DROP);
      ...
      ctx.beginPath();
      ctx.ellipse(this.x, this.y + 6 + CrewMember.BODY_DROP, 16, 8, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
      ctx.font = '11px monospace';
      ctx.textAlign = 'center';
      ctx.fillStyle = this.decaying ? '#3aff6a' : this.dead ? '#98a0b8' : '#ffd700';
      ctx.fillText(this.decaying ? '☣' : this.dead ? '☠' : '✚',
                   this.x, this.y - 12 + CrewMember.BODY_DROP);
    ...
    /* Corpse plague — GREEN and pulsing (update68). The virus below is
       RED. They were the same green and the player could not tell a
       nuisance a medkit clears from a thing that kills a man in three
       fights. The colours come from ONE table in renderer.js, so the
       roster and the deck can never disagree about which is which. */
    if (this.infected && !this.virus) {
      const pulse = 0.55 + 0.45 * Math.sin((this._infT = (this._infT ?? 0) + 0.12));
      ctx.save();
      ctx.globalAlpha = pulse;
      ctx.font = '10px monospace';
      ctx.textAlign = 'center';
      ctx.fillStyle = (typeof Renderer !== 'undefined' && Renderer.DISEASE_COL)
        ? Renderer.DISEASE_COL.plague : '#3fd96b';
  ...
  serialise() {
    return {
      ...
      /* IN A SLAB, and for how long he has been in it (update77).
         Saved, because the whole promise of carbonite is that his
         clock is where he left it — and a reload that thawed him
         would tick it on while the player was not even playing. */
      frozen: this.frozen, _slabSeq: this._slabSeq, _slabRoom: this._slabRoom,
      ...
      /* WHAT HE IS WORTH WITHOUT A COMMANDER (update68).
       *
       * `Commander.reseatMaxHp` raises maxHp by the commander's bonus
       * and remembers the untouched figure in `baseMaxHp` so it can be
       * recomputed instead of stacked. That field was never SAVED — so
       * a man banked in the barracks came back with his RAISED maxHp
       * as his new base, and the same +2% was applied to it again next
       * contract. 100 → 102 → 104 → 106, one free point per run, for
       * as long as the commander kept his chair.
       *
       * The player caught it exactly: "czyli powiela co wyprawę, źle".
       */
      baseMaxHp: this.baseMaxHp ?? this.maxHp,
...
/* ── ONE NAME, ONE MAN (update54) ──────────────────────────
 *
 * Every name in this game was `Utils.pick(CREW_NAMES)` — thirty-two
 * names drawn with replacement, so by the birthday rule a barracks of
 * seven had better than even odds of holding two of the same, and the
 * player was left picking between two identical rows. Cats drew from
 * their own list the same way.
 *
 * This is the only place a name is drawn now. `taken` is passed in by
 * the caller, because the list of names in use is not a fact this file
 * owns — it lives in the barracks, the pens and the hull, and a
 * registry kept in here would be a second copy of it that goes stale
...
function pickUniqueName(pool, taken = []) {
  ...
  if (free.length) return Utils.pick(free);
  const ROMAN = ['II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'];
  const base = Utils.pick(pool && pool.length ? pool : ['Hand']);
  for (const r of ROMAN) {
    const tryName = `${base} ${r}`;
    if (!used.has(tryName)) return tryName;
  }
  // Eleven of one name aboard is not a case worth carrying code for,
  // but it must still not hand back a duplicate.
  let n = 11;
...
function makeCat(kind = null, name = null, taken = []) {
