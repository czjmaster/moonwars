...
const Assets = (() => {
  ...
  /* ══ DRAWN ART, WHEN THERE IS ANY (update73a) ═════════════
   *
   * Every sprite in this file is generated in code, and that is the
   * FLOOR, not a placeholder to be ripped out: the game has to run
   * with zero image files, because today it has zero image files and
   * on the day one of them fails to load it still has to boot.
   *
   * So a file never replaces a generator — it OVERRIDES one. Anything
   * without a file keeps the drawing this module makes for it, which
   * means art can arrive one icon at a time instead of all at once.
   *
   * THE MANIFEST IS WHY THIS IS QUIET. Probing for every sprite name
   * would fire forty requests and print forty 404s into the console
   * of a game that is working perfectly. Instead there is ONE request
   * — `art/manifest.json` — and if it is not there, nothing else is
   * asked for. That is today's state: one 404 at boot, or none at all
   * once the file exists and lists nothing.
   *
   *     art/manifest.json   ["icon_medbay", "room_default", …]
   *     art/icon_medbay.png
   *
   * NOTHING HERE MAY HANG THE BOOT. Every wait has a timeout and
   * every failure is silent-by-design — a missing file is the normal
   * case, not an error. The test harness has an `Image` that never
   * fires `onload` or `onerror` at all, which is exactly the shape of
   * a boot that never finishes, so the timeout is not belt-and-braces:
   * it is the thing being relied on.
   */
  const ART_DIR      = 'art/';
  const ART_TIMEOUT  = 4000;   // ms, per file and for the manifest
  /** name → 'file' | 'generated'. What the player is actually seeing. */
  const _source = new Map();

  /** Resolve to `value` if `p` has not settled within `ms`. Never rejects. */
  function _withTimeout(p, ms, value = null) {
    return new Promise(resolve => {
      let done = false;
      const finish = (v) => { if (!done) { done = true; resolve(v); } };
      const t = setTimeout(() => finish(value), ms);
      Promise.resolve(p).then(v => { clearTimeout(t); finish(v); },
                              () => { clearTimeout(t); finish(value); });
    });
  }

  /** The list of sprites that have a drawn file, or [] for none. */
  async function _readManifest(ms = ART_TIMEOUT) {
    if (typeof fetch !== 'function') return [];
    const res = await _withTimeout(
      fetch(ART_DIR + 'manifest.json', { cache: 'no-cache' }), ms);
    if (!res || !res.ok) return [];
    const body = await _withTimeout(res.json(), ms);
    /* TWO SHAPES ACCEPTED, and that is deliberate. A bare array is the
       short form; an object with `sprites` lets the file carry a note
       explaining itself, because JSON has no comments and the person
       who opens this next should not have to come here to find out
       what it is for. */
    const list = Array.isArray(body) ? body
               : Array.isArray(body?.sprites) ? body.sprites
               : [];
    return list.filter(n => typeof n === 'string');
  }

  /** One drawn file, or null. Resolves either way — never throws. */
  function _loadImage(name, ms = ART_TIMEOUT) {
    if (typeof Image !== 'function') return Promise.resolve(null);
    return _withTimeout(new Promise(resolve => {
      const img = new Image();
      img.onload  = () => resolve(img);
      img.onerror = () => resolve(null);
      img.src = ART_DIR + name + '.png';
    }), ms);
  }

  /**
   * Swap in whatever art exists. Called after the generators have run,
   * so every name already has something to fall back to.
   *
   * Loads in PARALLEL and swallows everything: a hull that boots in
   * two seconds without art must not boot in forty with it.
   */
  async function _loadDrawnArt(onProgress, ms = ART_TIMEOUT) {
    let names = [];
    try { names = await _readManifest(ms); } catch (_) { names = []; }
    if (!names.length) return 0;

    let done = 0;
    const results = await Promise.all(names.map(async name => {
      const img = await _loadImage(name, ms);
      done++;
      if (onProgress) onProgress(done / names.length, `Art ${name}…`);
      if (!img || !img.width) return false;
      _sprites.set(name, img);
  ...
  async function init(onProgress) {
    const tasks = [
      ...
      ['icon_carbonite',      () => _genSystemIcon('carbonite')],
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      _source.set(name, 'generated');
      if (onProgress) onProgress((i + 1) / tasks.length, `Loading ${name}…`);
      // Yield to browser between tasks
      await new Promise(r => setTimeout(r, 0));
    }

    /* Drawn art last, so every name has a generated sprite to fall
       back to before anything is allowed to override it. Wrapped
       because a broken manifest must cost the player a plainer
       looking game, never a game that will not start. */
    try { await _loadDrawnArt(onProgress); } catch (_) { /* generated art stands */ }
  }
  ...
  /** Every name that has a drawn file behind it right now. */
  function fromFiles() {
