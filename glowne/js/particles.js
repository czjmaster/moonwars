...
const Particles = (() => {
  ...
  /** @param spread {speedMin,speedMax,angleMin,angleMax} — the angle
   *  range makes a CONE (used by the muzzle flash); leave it out for a
   *  full circle. */
  function burst(count, baseCfg, spread = {}) {
    for (let i = 0; i < count; i++) {
      const angle = (spread.angleMin !== undefined || spread.angleMax !== undefined)
        ? Utils.randFloat(spread.angleMin ?? 0, spread.angleMax ?? Math.PI * 2)
  ...
  /** Muzzle flash at a gun port — a short, bright cone of sparks so a
   *  shot READS as leaving your ship instead of just appearing. */
  function muzzleFlash(x, y, dir = 1, color = '#ffd780') {
    burst(7, {
      x, y, color: '#ffffff', colorEnd: color,
      size: 3.5, sizeEnd: 0, life: 0.16, ay: 0,
    }, { speedMin: 60, speedMax: 190, angleMin: -0.35, angleMax: 0.35 });
    emit({
      x: x + dir * 5, y, vx: dir * 20, vy: 0, ay: 0,
      color, size: 9, sizeEnd: 0, life: 0.13, alpha: 0.9, alphaEnd: 0,
    });
  }

  /** Lazy smoke from a wrecked module — thin, slow, and it drifts up. */
  function damageSmoke(x, y) {
    emit({
      x: x + Utils.randFloat(-6, 6), y: y + Utils.randFloat(-4, 4),
      vx: Utils.randFloat(-6, 6), vy: Utils.randFloat(-18, -8), ay: -4,
      color: '#5a6070', colorEnd: '#20242e',
      size: Utils.randFloat(4, 9), sizeEnd: 14,
      life: Utils.randFloat(0.9, 1.6), alpha: 0.35, alphaEnd: 0,
    });
  }
