...
const MISSIONS = {
  ...
  /* ── TWO CONTRACTS AT THE EDGES OF KARMA (update71) ──────────
   *
   * The player's call, and the right one (VARIANT A): the three jobs
   * above are a DIFFICULTY LADDER — one, two and three sectors — not a
   * menu of flavours. Putting a karma gate on any of them would take
   * the game away from a commander instead of giving him a choice. So
   * nothing above ever closes, and these two open.
   *
   * They are deliberately the same length as Border Patrol, because
   * the thing being chosen here is WHAT KIND OF WORK you take, not how
   * long you are out. One pays badly and pays you in your name; the
   * other pays well and costs it.
   *
   * The thresholds are `Chips.wallColumn` — the five karma bands the
   * CPU board already runs on. No new numbers: the wall a commander
   * can see on his own board is the same line the yard reads.
   */
  relief: {
    id: 'relief',
    label: 'Relief Run',
    ...
    boss: null,
    difficulty: 'easy',
    ccBonus: 20,                 // they have almost nothing to give
    karmaGate: 'high',
    /* PAID IN THE ONLY CURRENCY THEY HAVE. `RESCUE_AT_COST` is the
       existing figure for "saving people at the expense of the run" —
       which is precisely what this contract is. */
    karmaPay: 'RESCUE_AT_COST',
    chipReward: true,            // and whatever was left in their lab
    blurb: 'A moon with no yard and no money. Two sectors, no boss, '
         + 'and they can only thank you.',
  },
  runners: {
    id: 'runners',
    ...
    sectors: 2,
    boss: 'elite',
    ccBonus: 160,                // this is why anybody takes it
    karmaGate: 'low',
    /* WHAT IT COSTS. `ROBBERY` is the table's entry for taking from
       people who cannot stop you, and that is who is at the other end
       of this cargo. Charged on COMPLETION, like the CC: signing is
       not the deed, delivering is. */
    karmaPay: 'ROBBERY',
    blurb: 'Sealed crates, a fixed course and a warlord\'s escort at the '
         + 'end of it. Nobody asks what is in them.',
  },
};

/**
 * WHY A CONTRACT IS NOT ON THE BOARD TODAY (update71).
 *
 * Returns null when it is open, or the sentence to put on screen.
 * A job that is simply missing reads as a bug — the closed dock of
 * update61 taught that lesson already, and this is the same rule:
 * what is refused says so, in words, where the player is looking.
 */
function missionRefusal(m, karma = null) {
  if (!m || !m.karmaGate) return null;
  const k = (karma != null) ? karma
    : ((typeof Commander !== 'undefined' && Commander.karmaNow)
        ? Commander.karmaNow() : 50);
  const col = (typeof Chips !== 'undefined' && Chips.wallColumn)
    ? Chips.wallColumn(k) : 3;
  if (m.karmaGate === 'high' && col < 4) {
    return 'They will not hand their people to a commander with your name.';
  }
  if (m.karmaGate === 'low' && col > 2) {
    return 'Nobody offers this to a commander with a clean record.';
  }
  return null;
}
...
const Base = (() => {
  ...
  /**
   * RENAME (update54). One door for a hand, a cat and a commander,
   * because the player does not think of them as three lists — he
   * thinks of the name under the portrait he just clicked.
   *
   * The new name is trimmed, capped, and REFUSED if somebody else is
   * already using it: uniqueness that only the random draw respects is
   * not uniqueness. Renaming somebody to what he is already called is
   * allowed and does nothing, so the field can be left alone.
   */
  const NAME_MAX = 14;
  function renameCrew(id, name) {
    const clean = String(name ?? '').replace(/\s+/g, ' ').trim().slice(0, NAME_MAX);
    if (!clean) return { ok: false, message: 'A name cannot be empty.' };
    const b = get();
    const lists = [b.barracks ?? [], b.pets ?? [], b.commanders ?? []];
    const rec = lists.map(l => l.find(r => r && r.id === id)).find(Boolean);
    if (!rec) return { ok: false, message: 'Nobody by that number.' };
    if (rec.name === clean) return { ok: true, message: `${clean} it is.`, name: clean };
    const clash = lists.flat().some(r => r && r.id !== id && r.name === clean);
    if (clash) return { ok: false, message: `${clean} is already somebody aboard.` };
    rec.name = clean;
    _commit();
  ...
  /* EVERY NAME THE BASE IS ALREADY USING (update54) — barracks, pens
     and the mess, read fresh each time. A new hand or a new cat is
     drawn against this, so no two rows on a list ever read the same.
     Computed, never stored: a kept list would go stale the first time
     somebody died. */
  function takenNames() {
    const b = get();
    return [
  ...
  function adoptCat(kind = null) {
    ...
    spend(PRICE.cat);
    const cat = makeCat(kind || Utils.pick(['black', 'ginger']), null, takenNames());
  ...
  function uninstallWeapon(shipIndex, slot) {
    ...
    /* THE SHELF IS CHECKED BEFORE THE BOLTS COME OFF (update54).
       This used to unbolt the gun, commit the hull WITHOUT it, and only
       then try to find shelf space — so a full warehouse DESTROYED the
       weapon and reported the failure after the fact.

       Since update75 that rule lives INSIDE `Ship.uninstallWeapon`, and
       this call simply hands it the shelf to box into: the gun comes off
       the mount only if the crate lands. The scratch copy is still a
       scratch copy — nothing is committed until the move succeeded. */
    const g = warehouseGrid();
    if (!g || !getWeaponDef(w.defKey) || !ship.uninstallWeapon(slot, g)) {
  ...
  function sellShip(index) {
    ...
    /* AND THE GUNS IN HER HOLD (update75). They used to be read off
       `weaponCargo`; a boxed gun is now an ordinary crate in the grid,
       so that is where the yard looks. Selling a hull must not quietly
       scrap the spare gun riding in her. */
    (sold.cargo?.items ?? [])
      .filter(it => it.def?.kind === 'weapon' && it.meta)
      .forEach(it => { if (!storeWeapon(it.meta)) lost++; });
  ...
  function launch({ shipIndex = 0, crewIds = [], commanderId = null, petId = null,
    ...
    /* AND THE MAN IN THE CHAIR HAS TO BE THE RIGHT MAN FOR IT. Checked
       at the LAUNCH, not only on the card: karma moves during a run, so
       a contract picked yesterday can be one he no longer qualifies
       for — and the reason is the card's own sentence, not a second
       way of saying it. */
    {
      const why = missionRefusal(MISSIONS[mission]);
    ...
    /* WHAT WENT OUT LAST TIME (update54).
       The hangar screen used to reset to hull 0 and re-tick the first
       four names in the barracks EVERY time the base opened, so a
       player who flew the Horus with a hand-picked crew came home to
       the starter scout and a roster somebody else had chosen. Worse,
       the list order shifts as people are hired and lost, so the "first
       four" were not even the same four twice — that is the ticking and
       unticking by itself the player saw.
       Two remembered facts, written at the one moment the choice is
       final, and read back as a DEFAULT the player can still change.
       The hull is remembered by KEY, not index: she is spliced out of
       the hangar for the flight and pushed back on the end when she
       docks, so no index survives the trip. */
    b.lastShipKey = b.ships[shipIndex]?.key ?? null;
    b.lastCrew    = roster.map(c => c.id);
  ...
  function returnFromRun({ shipEntry = null, crew: crewData = [], fuel = 0, missiles = 0, cc: ccEarned = 0 } = {}) {
    ...
    /* THE SPARE-GUN SPECIAL CASE IS GONE (update75). It emptied
       `shipEntry.data.weaponCargo` onto the armoury shelf — a rule
       written just for guns, because guns were the one thing aboard
       that did not live in the hold. They live in the hold now, and
       the hold is sorted onto the shelf at the dock like everything
       else, so the general rule already does this. Deleting the
       special case is the fix; adding a second one would have been
       the bug.

       `report.gunsStored` went with it. It was counted here and read
       NOWHERE — not by the outcome screen, not by a test. A number
       nobody looks at is not a report, it is a place for two truths to
       disagree in private. */

    if (ccEarned > 0) { earn(ccEarned); report.cc = Math.round(ccEarned); }

    /* ── THE BOUNTY OFFICE (update63) ──────────────────────
     *
     * Prisoners and bagged bodies are paid out HERE, at the dock, and
     * are removed from the hull as they are paid — one place where a
     * man stops being cargo and becomes CC. Leaving them on the hull
     * and paying elsewhere would let the same man be handed in twice,
     * which is the shape of every double-count bug in this file's
     * history.
     */
    if (shipEntry && shipEntry.data) {
      const d = shipEntry.data;
      (d.prisoners ?? []).forEach(p => {
        const paid = Math.max(0, Math.round(p.bounty ?? 0));
        if (paid > 0) earn(paid);
        report.bounty += paid;
        report.prisoners++;
      ...
      // Bagged bodies ride in the hold, and each bag carries its own price.
      const hold = d.cargo;
      if (hold && Array.isArray(hold.items)) {
...
if (typeof window !== 'undefined') {
  ...
  window.missionRefusal = missionRefusal;
}
