...
const Base = (() => {
   ...
   *
   * The CPU board cannot be looked at without a captain, and a
   * captain cannot be had without a crewman who has MASTERED a skill
   * — eight to ten fights of dedicated work, by design. That is the
   * right rule for the game and a terrible one for looking at a
   * screen that was just built, so this is the back door, and it is
   * labelled as one everywhere it appears.
   *
   * It does NOT relax the real rule: `promote()` is untouched, still
   * costs 100 CC and still demands mastery. This is a separate door
   * with TEST written on it. Read §13 of the spec before turning it
   * into a feature — "buying ready-made captains" is on the list of
   * things deliberately rejected.
   */
  function devCaptain(opts = {}) {
    const b = get();
    if (typeof Captain === 'undefined') return { ok: false, message: 'Captains unavailable.' };
    if ((b.captains ?? []).length >= Math.max(1, messCap()))
      return { ok: false, message: 'No free berth in the mess.' };

    // Take a real crewman if there is one, so his name and corporation
    // are a real record; otherwise invent one rather than fail.
    const rec = (b.barracks ?? [])[0] || new CrewMember({}).serialise();
    const cap = Captain.fromCrew(rec);
    cap.level = Utils.clamp(opts.level ?? 8, 1, Captain.MAX_LEVEL);
    cap.karma = Utils.clamp(opts.karma ?? 50, 0, 100);
    if ((b.barracks ?? []).some(c => c.id === rec.id)) {
      b.barracks = b.barracks.filter(c => c.id !== rec.id);
    }
    (b.captains = b.captains ?? []).push(cap);
    _commit();
    return { ok: true, captain: cap,
             message: `TEST: ${cap.name} na mostku, poziom ${cap.level}, karma ${cap.karma}.` };
  }

  /**
   * A spread of chips on the shelf, for the same reason: one of each
   * family at levels I–III, so every rule on the board — sides, the
   * wall, a bar that will not fit, a 2x2 that needs two rows — can be
   * tried in one sitting.
   */
  function devChips() {
    const g = warehouseGrid();
    if (!g || typeof Chips === 'undefined') return { ok: false, message: 'Cargo system not loaded.' };
    let n = 0;
    ['life_reserve', 'golden_hour', 'assault_squad', 'forced_fire',
     'mobility', 'deck_service', 'escape_pod'].forEach(key => {
      for (let lvl = 1; lvl <= 3; lvl++) {
        if (g.add(Chips.itemKey(key, lvl))) n++;
      }
    });
    commitWarehouse(g);
    return { ok: !!n, message: n ? `TEST: ${n} chipów na półce.` : 'Półka pełna.' };
  }

  /** Nudge a captain's karma. update50 gives this real sources. */
  function devKarma(capId, delta) {
    const cap = captainById(capId);
    if (!cap) return { ok: false, message: 'No such captain.' };
    cap.karma = Utils.clamp((cap.karma ?? 50) + delta, 0, 100);
    saveCaptain(cap);
    return { ok: true, captain: cap, message: `TEST: karma ${cap.karma}.` };
  }
