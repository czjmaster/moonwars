...
const Base = (() => {
  ...
  function uninstallWeapon(shipIndex, slot) {
    ...
    /* THE SHELF IS CHECKED BEFORE THE BOLTS COME OFF (update54).
       ...
       weapon and reported the failure after the fact. The order is now
       the other way round: everything that has to be stowed is laid out
       on a scratch copy of the shelf first, and the gun only leaves the
       mount once every crate has a place. Nothing is committed until
       both sides of the move can succeed. */
    const pending = [...(ship.weaponCargo ?? []), w.defKey];
    const g = warehouseGrid();
    const crateFor = (k) => (typeof cargoCrateForWeapon === 'function')
      ? cargoCrateForWeapon(k) : 'gun_crate';
    if (!g || !pending.every(k => getWeaponDef(k) && g.add(crateFor(k), k))) {
      return { ok: false, message: 'No room on the shelf for that gun — clear some space first.' };
    }

    ship.uninstallWeapon(slot);
    ship.weaponCargo = [];        // every one of them is on the shelf now
    commitWarehouse(g);
    entry.data = ship.serialise();
    _commit();
