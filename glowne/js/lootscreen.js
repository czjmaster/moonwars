...
const LootScreen = (() => {
  ...
  function _putDown(mx, my, CELL) {
    ...
    for (const which of ['wreck', 'hold']) {
      ...
      /* AND IT MAY NOT BE PUT DOWN ON THEIR SIDE EITHER. Blocking the
         till and leaving the shelf open would just be a slower way of
         handing the same crate over. */
      if (_opts.locked?.(it) && r.grid !== _hold) {
        _say(_opts.lockedWhy || 'they will not take that one', false);
        continue;
      }
  ...
  function _act(act, arg) {
    ...
    switch (act) {
      ...
      case 'sell': {
        ...
        /* SOME CRATES THE OTHER SIDE WILL NOT SIGN FOR (update69) —
           the home yard and a sealed egg case, for one. Refused with
           the reason on screen rather than greyed out in silence. */
        if (_opts.locked?.(it)) {
          _say(_opts.lockedWhy || 'they will not take that one', false);
          return null;
        }
      ...
      case 'done': {
        if (_opts.sellRestOnDone && _opts.onSell && _hold.items.length) {
          ...
          for (const it of [..._hold.items]) {
            // A locked crate is not part of "the rest": it stays aboard.
            if (_opts.locked?.(it)) continue;
            _hold.remove(it);
  ...
  return { openLoot, openHold, isOpen, update, draw,
           /** Seconds left on the salvage clock, or null when untimed.
            *  Read-only, and exposed so a test can ask the SCREEN how
            *  long the player has rather than re-deriving it. */
           secondsLeft: () => (_timed ? _timer : null),
           // exposed for tests — button positions shift as buttons are
           // added, so tests ask for them by name instead of guessing.
           _cellAt, _gridRect,
           /** The two grids this screen is editing, BY IDENTITY.
            *  A test that only compares contents cannot tell one shelf
            *  from a copy of it — and a copy of the shelf was exactly
            *  the CPU-chip duplication bug (update79). */
           _grids: () => ({ wreck: _wreck, hold: _hold }),
