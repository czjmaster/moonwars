 ...
 *   session 5 — the shop reads karma: surcharge banner and a shut dock
 *   session 6 — real sprite pixels: what lives in the duct has to fit
 *
...
async function session1(browser) {
  ...
  try {
    ...
    ok(await page.evaluate(() => typeof Commander !== 'undefined'),
       'commander.js loaded from index.html');
    ...
    ok(await page.evaluate(() => Base.commanders().length === 1),
    ...
    ok(await page.evaluate(() => BaseScreen._state().commanderId === Base.commanders()[0].id),
       'the new commander is the one flying the next contract');

    /* ── THE PROMOTION SCREEN IS IN THE WAY, ON PURPOSE (update52) ──
     * He arrives owing one bonus pick per level, and the screen opens
     * for them straight away rather than waiting behind a button.
     * That is the behaviour under test here — and then the picks have
     * to actually be spendable, or the base is unreachable for good. */
    await page.waitForTimeout(200);
    ok(await page.evaluate(() => Game.state() === 'promo'),
       'the promotion screen opens the moment he takes the chair');
    const owed = await page.evaluate(() =>
      Commander.picksOwed(Base.commanders()[0]));
    ok(owed >= 1, `and it owes him one pick per level (${owed})`);

    for (let i = 0; i < owed + 2; i++) {
      if (await page.evaluate(() => Game.state() !== 'promo')) break;
      await clickCanvas(page, 640, 428);       // the first of the two trades
      await page.waitForTimeout(120);
    }
    ok(await page.evaluate(() => Game.state() === 'base'),
       'spending the last pick hands the base back');
    ok(await page.evaluate(() => Commander.picksOwed(Base.commanders()[0]) === 0),
       'and every level he had is now spent on something');
    ok(await page.evaluate(() =>
         Commander.picksMade(Base.commanders()[0]) === Base.commanders()[0].level),
       'one pick per level, no more and no fewer');
    ...
    ok(await page.evaluate(() => !!Save.getRun().commanderId),
       'and the commander sails with it');
    ok(await page.evaluate(() => (Base.commanders()[0] || {}).away === true),
...
async function session2(browser) {
  ...
  const LATE = ['js/base.js', 'js/basescreen.js', 'js/cargo.js',
                'js/lootscreen.js', 'js/wreck.js', 'js/commander.js'];
  let stale = real;
  LATE.forEach(src => {
  ...
  try {
    ...
    const healed = await page.evaluate(() =>
      ['Base', 'BaseScreen', 'CargoGrid', 'LootScreen', 'DockingGame', 'Commander']
...
// ════════════════════════════════════════════════════════════
// SESSION 5 — the shop reads karma (update61)
// ════════════════════════════════════════════════════════════

async function session5(browser) {
  section('SESSION 5 — the shop reads karma: surcharge banner and a shut dock');
  const host = await serve();
  const page = await browser.newPage({ viewport: { width: 1280, height: 720 } });
  const errors = watchErrors(page, host.url);
  ...
  try {
    ...
    /* Pick the seeds HERE, in the page, from the same rule the game
       uses — hard-coding "seed 7 refuses" would make this test a
       hostage of the seed arithmetic rather than a test of the shop. */
    const seeds = await page.evaluate(() => {
      let shut = null, open = null;
      for (let s = 0; s < 40 && (shut === null || open === null); s++) {
        if (Commander.portRefuses(s, 0)) { if (shut === null) shut = s; }
        else if (open === null) open = s;
      }
      return { shut, open };
    });
    ok(seeds.shut !== null && seeds.open !== null,
       `some ports shut a shunned commander out and some do not (${seeds.shut} / ${seeds.open})`);

    const openShop = (karma, seed) => page.evaluate(({ karma, seed }) => {
      Save.load(); Save.startRun();
      Save.updateRun({ scrap: 9999 });
      const cap = Commander.fromCrew({ id: 'bk', name: 'Rook', race: 'terra', skills: {} });
      cap.karma = karma;
      Commander.setActive(cap);
      const ship = new Ship('frigate', true, 80, 120);
      ship._allocateDefaultPower();
      ship.hull = Math.max(1, ship.hullMax - 20);
      const station = new Station(2, seed);
      station.stock.hullRepair = 40;
      window.__shop = { ship, station };
      UI.openStation(station, ship);
    }, { karma, seed });

    // ── AN ORDINARY COMMANDER: no banner at all ──────────────
    await openShop(50, seeds.open);
    await page.waitForTimeout(150);
    ok(await page.evaluate(() => !document.getElementById('station-karma-banner')),
       'an ordinary commander sees no banner — the shop is unchanged for him');
    const parPrice = await page.evaluate(() => window.__shop.station.hullRepairCost(5));

    // ── A DISTRUSTED ONE: the surcharge is said out loud ─────
    await openShop(30, seeds.open);
    await page.waitForTimeout(150);
    const warned = await page.evaluate(() => {
      const b = document.getElementById('station-karma-banner');
      return { text: b ? b.textContent : null,
               tabs: document.querySelectorAll('.station-tab').length,
               price: window.__shop.station.hullRepairCost(5) };
    });
    ok(warned.text && /DISTRUSTED/.test(warned.text),
       `the banner names him: "${warned.text}"`);
    ok(/10%/.test(warned.text), 'and gives the figure, not just an adjective');
    ok(warned.tabs > 0, 'the shop stays OPEN to him — a surcharge is not a refusal');
    ok(warned.price > parPrice,
       `and the yard really charges it (${warned.price} vs ${parPrice} CC for 5 HP)`);

    /* THE LABEL ON THE BUTTON IS THE PRICE THAT WILL BE TAKEN. The
       whole reason the shop asks the station for a quote instead of
       multiplying its own copy. */
    await page.click('.station-tab[data-tab="repair"]');
    await page.waitForTimeout(200);
    const label = await page.evaluate(() => {
      const s = window.__shop.station;
      const want = `+5 HP — ${s.hullRepairCost(5)} CC`;
      const seen = [...document.querySelectorAll('#station-content span')]
        .map(e => e.textContent.trim()).filter(t => /^\+5 HP/.test(t));
      return { want, seen };
    });
    ok(label.seen.includes(label.want),
       `the repair button quotes the karma price ("${label.want}" among ${JSON.stringify(label.seen)})`);

    // ── A SHUNNED ONE AT A DOCK THAT REFUSES HIM ─────────────
    await openShop(5, seeds.shut);
    await page.waitForTimeout(150);
    const shut = await page.evaluate(() => {
      const b = document.getElementById('station-karma-banner');
      return { text: b ? b.textContent : null,
               tabs: document.querySelectorAll('.station-tab').length,
               content: !!document.getElementById('station-content'),
               depart: !!document.getElementById('station-close-btn') };
    });
    ok(shut.text && /DOCK CLOSED/.test(shut.text),
       `the shut dock says so on screen: "${shut.text}"`);
    ok(/will not trade/.test(shut.text), 'and gives the reason, so it does not read as a bug');
    ok(shut.tabs === 0 && !shut.content,
       'and shows no shop at all rather than an empty one');
    ok(shut.depart, 'DEPART is still there — the player is never trapped in a closed port');

    // ── THE SAME COMMANDER AT A DOCK THAT STILL DEALS ────────
    await openShop(5, seeds.open);
    await page.waitForTimeout(150);
    ...
    ok(still.tabs > 0, 'a port that will deal with him is open for business');
    ok(still.text && /NOTORIOUS/.test(still.text) && !/DOCK CLOSED/.test(still.text),
       `but it charges him for the name: "${still.text}"`);

    await page.evaluate(() => Commander.setActive(null));
    await page.click('#station-close-btn');
    await page.waitForTimeout(200);

    ok(errors.length === 0, 'no page errors in the karma shop'
       + (errors.length ? ':\n       ' + errors.join('\n       ') : ''));
  } finally {
    await page.close();
    await host.close();
...
// ════════════════════════════════════════════════════════════
// SESSION 6 — what lives in the duct has to fit in the duct
// ════════════════════════════════════════════════════════════

/*
 * This one can only be asked here. The sprites are drawn onto real
 * canvases at boot and the Node harness has a canvas that draws
 * nothing, so "is the cat taller than the duct" is a question with no
 * answer outside a browser — and update73 got it wrong by eye, picking
 * a one-tile duct that the spider did not fit in standing still.
 *
 * So: render every beast frame at the size the game actually draws it,
 * count the non-transparent pixels, and compare the tallest against
 * VENT_H. Redraw the cat and this fails on the day it is redrawn, not
 * on the day somebody notices it clipping.
 */
async function session6(browser) {
  section('SESSION 6 — the duct is tall enough for what will live in it');
  const host = await serve();
  const page = await browser.newPage({ viewport: { width: 1280, height: 720 } });
  const errors = watchErrors(page, host.url);
  ...
  try {
    ...
    const out = await page.evaluate(() => {
      /* The size CrewMember.draw uses — read from the source of truth
         rather than typed in here, so the two cannot drift. */
      const SCREEN = 32;
      const ink = (frame) => {
        const c = document.createElement('canvas');
        c.width = SCREEN; c.height = SCREEN;
        const g = c.getContext('2d');
        g.drawImage(frame, 0, 0, SCREEN, SCREEN);
        const d = g.getImageData(0, 0, SCREEN, SCREEN).data;
        let top = 1e9, bot = -1;
        for (let y = 0; y < SCREEN; y++) {
          for (let x = 0; x < SCREEN; x++) {
            if (d[(y * SCREEN + x) * 4 + 3] > 12) {
              if (y < top) top = y;
              if (y > bot) bot = y;
            }
          }
      ...
      const tallest = (mk) => {
        let h = 0;
        ['idle', 'walk', 'fight'].forEach(mode => {
          (mk(mode).frames || []).forEach(f => { h = Math.max(h, ink(f)); });
        });
        return h;
      };
      return {
        vent:   HULL_GRID.VENT_H,
        tile:   HULL_GRID.TILE,
        rat:    tallest(m => Animation.ratAnim(m, '#b3a189')),
        cat:    tallest(m => Animation.catAnim(m, '#3a3a42')),
        spider: tallest(m => Animation.spiderAnim(m, '#9fff7a')),
      };
    });

    ok(out.vent > 0, `the duct has a height (${out.vent}px)`);
    ['rat', 'cat', 'spider'].forEach(k => {
      ok(out[k] > 0, `${k}: the sprite draws something (${out[k]}px of ink)`);
      ok(out[k] <= out.vent,
         `${k} fits in the duct (${out[k]}px into ${out.vent}px)`);
    });

    /* AND IT IS NOT WASTEFULLY TALL EITHER. A duct three tiles high
       would pass the test above and eat the compartment; the point is
       that it is the SMALLEST whole number of tiles that works. */
    const need = Math.max(out.rat, out.cat, out.spider);
    ok(out.vent - need < out.tile,
       `and is the smallest duct that does (${out.vent}px for ${need}px of cat)`);

    ok(errors.length === 0, 'no page errors while measuring'
       + (errors.length ? ':\n       ' + errors.join('\n       ') : ''));
  } finally {
    await page.close();
    await host.close();
...
(async () => {
  ...
  try {
    ...
    await session5(browser);
    await session6(browser);
  } catch (e) {
    failed++;
