...
const GAME_TEST_EXPORT = `return { init, hasCommander: _hasCommander, state: () => STATE, __test: {
  ${['_makeParty', '_updateParty', '_drawParty', '_drawCombat', '_updateCombat',
     ...
     '_draw', '_update', '_updateMap', '_loop', '_cycleCrew', '_mapView',
     '_step', '_canPause', '_drawPause', '_relieveInRoom', '_openCpuBoard'].map(T_REF).join(',\n  ')},
  get paused() { return _paused; },          set paused(v) { _paused = v; },
  ...
  get counterBoarded() { return _counterBoarded; },
  set counterBoarded(v) { _counterBoarded = v; },
  get bodyMenu() { return typeof _bodyMenu !== 'undefined' ? _bodyMenu : undefined; },
  set bodyMenu(v) { try { _bodyMenu = v; } catch (e) {} },
