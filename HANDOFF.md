...
## 5-0. ZMIANY update79 (NAJNOWSZE — WALKA W MODULE)
...
## 5-0a. ZMIANY update78 (OPATRUNEK ≠ MEDBAY)
...
## 5-0c. ZMIANY update76 (PAUZA I PASEK ZNACZNIKÓW)
...
## 5-0d. ZMIANY update75 (BROŃ JAKO SKRZYNIA)
...
## 5-0e. ZMIANY update74 (ZGŁOSZENIA Z ŻYWEJ GRY)
...
## 5-0f. ZMIANY update73a (KANAŁ NA DWA KAFLE I RYSOWANA GRAFIKA)
...
## 5-0g. ZMIANY update73 (KAFELKI, KANAŁ I KADŁUB Z PROFILEM)
...
## 5-0h. ZMIANY update72 (LUDZIE, KTÓRZY NIE SĄ ZAŁOGĄ)
...
## 5-0i. ZMIANY update71 (CELE DODATKOWE I DWA KONTRAKTY)
...
## 5-0j. ZMIANY update70 (CZARNY RYNEK, ADRES NA PLAKACIE, GŁÓD)
...
## 5-0k. ZMIANY update69 (DRZWI, JAJO I ZEGAR)
...
## 5-0l. ZMIANY update68 (DZIESIĘĆ ZGŁOSZEŃ Z ŻYWEJ GRY)
...
## 5-0m. ZMIANY update67 (SPŁATA DŁUGÓW)
...
## 5-0n. ZMIANY update66 (POLOWANIE NA MAPIE + CZTERY RACJE)
...
## 5-0o. ZMIANY update65 (CIAŁO TO DECYZJA, NIE SPRZĄTANIE)
...
## 5-0p. ZMIANY update64 (LISTA GOŃCZA)
...
## 5-0q. ZMIANY update63 (CELA, JENIEC I CIAŁO W WORKU)
...
## 5-0r. ZMIANY update62 (NIKT SIĘ NIE PODDAJE RZEŹNIKOWI)
...
## 5-0s. ZMIANY update61 (ŚWIAT CZYTA KARMĘ)
...
## 5-0t. ZMIANY update60 (DZIAŁO PAMIĘTA SWOJĄ KOMORĘ)
...
## 5-0u. ZMIANY update59 (DZIAŁO, KTÓRE NIE STRZELA, MÓWI DLACZEGO)
...
## 5-0v. ZMIANY update58 (He-3 WIDOCZNY, KSIĘŻYCE DO ODKRYCIA)
...
## 5-0w. ZMIANY update57 (ZAŁOGA NIE ZMIENIA KORPORACJI, CONTINUE W BAZIE)
...
## 5-0x. ZMIANY update56 (He-3, MOON GATE, REGIONY)
...
## 5-0y. ZMIANY update55 (DRUGA TURA POPRAWEK Z TESTÓW NA ŻYWO)
...
## 5-0z. ZMIANY update54 (15 POPRAWEK Z TESTÓW GRACZA NA ŻYWO)
...
## 5-0aa. ZMIANY update53 (8 ROZKAZÓW SPECJALNYCH, WSZYSTKIE ROZKAZY W JEDNYM MIEJSCU)
...
## 5-0ab. ZMIANY update52a (SPECJALIZACJE WYJAŚNIONE, TECZKA DOWÓDCY, UI MESY)
...
## 5-0ac. ZMIANY update52 (25 RANG, DOWÓDCA 1–24, PLANSZA PO KWADRACIE, GRA PO ANGIELSKU)
...
## 5-0ad. ZMIANY update51 (2× XP, ROZKAZY POD KAPITANEM, AWANS ZE SUFITEM)
...
## 5-0ae. ZMIANY update50 (KARMA MA ŹRÓDŁA, KAPSUŁA LATA, WRÓG MA KAPITANA)
...
## 5-0af. ZMIANY update49a (STÓŁ TESTOWY DO PLANSZY CPU)
...
## 5-0ag. ZMIANY update49 (PLANSZA CPU, CHIPY, KARMA JAKO GEOMETRIA)
...
## 5-0ah. ZMIANY update48 (EKRAN ŁADOWNI: KLIK, PODZIAŁ STOSU, NIC NIE ZNIKA)
...
## 5-0ai. ZMIANY update47 (SKAFANDRY, POWIETRZE, GŁÓD, KOTY)
...
## 5-0aj. ZMIANY update46 (CARGO RETROFIT SKASOWANY)
...
## 5-0ak. ZMIANY update45 (KOTY KSIĘŻYCOWE)
...
## 5-0al. ZMIANY update44 (raport gracza: exploit z osłonami, mesa jako budynek)
...
## 5-0am. ZMIANY update43 (KAPITAN, mesa, XP z konsoli, korporacje wrogów)
...
## 5-0an. ZMIANY update42 (raport z testów gracza: zaraza, ranni, dźwięk, abordaż)
...
## 5-0ao. ZMIANY update41 (JEDNA SIATKA KADŁUBOWA, kafle silnika i dziobu)
...
## 5-0ap. ZMIANY update40 (AUDYT: ekran opcji, martwe mechaniki, wycieki stanu, UI)
...
## 5-0aq. ZMIANY update39 (He2 jako ładunek, mgła na mapie, szczury księżycowe, HP w koszarach)
...
## 5-0ar. ZMIANY update38 (zapis postępu sektora, sloty w modułach, combat tylko wręcz, jaja w różnych pokojach)
...
## 5-0as. ZMIANY update37 (chodzenie po podłodze, wraki z powietrzem, ukryte jaja, mniej chromu)
...
## 5-0at. ZMIANY update36 (hakowanie drzwi, sporne moduły, cmentarz z historią służby, kontrakt startowy)
...
## 5-0au. ZMIANY update35 (JEDEN MAGAZYN, klasy broni, cmentarz, 6 bugów załogi)
...
## 5-0av. ZMIANY update34 (WAREHOUSE wchłonięty przez SUPPLY, UI bazy, załoga przy konsolach, 3 realne bugi)
...
## 5-0aw. ZMIANY update33 (magazyn bazy jako prawdziwa siatka)
...
## 5-0ax. ZMIANY update32 (kolory ładowania, reaktor jako moduł, przebudowa UI bazy)
...
## 5-0ay. ZMIANY update31 (jaja we wrakach, sprite'y pająków, grafika broni, nazwy egipskie)
...
## 5-0az. ZMIANY update30 (bilans modułów, drzwi na czas, grafika broni, naprawa w bazie)
...
## 5-0ba. ZMIANY update29 (broń tylko w mount albo w skrzyni, kolory korporacji, martwe wraki, hangar)
...
## 5-0bb. ZMIANY update28 (dokowanie, wraki po których się chodzi, pająki i wirus)
...
## 5-0bc. ZMIANY update27 (łączenie stosów, skrytka na broń, winda po abordażu)
...
## 5-0bd. ZMIANY update26 (STOSY: ilość JEST przedmiotem)
...
## 5-0be. ZMIANY update25 (amunicja i broń w ładowni, salwy, więcej wraków)
...
## 5-0bf. ZMIANY update24 (ładownia siatkowa + ekran łupu)
...
## 5-0bg. ZMIANY update23 (UI portów + oprawa graficzna)
...
## 5-0bi. ZMIANY update21 (hotfix + nowy typ testów)
...
## 5-0bj. ZMIANY update20 (DUŻA: meta-progresja)
...
| 80 | **CO WIDAĆ** — pełny zestaw ikon statusu (naprawia, gasi, walczy, brak powietrza), **większe** ikony, JEDEN zestaw dla listy i pokładu, opatrunki i jedzenie na górnym pasku, **pociski z działek** | zgłoszenia gracza 24.09 |
| 81 | **JEDZENIE** — automat przy głodzie, od najgorszej potrawy; przerywa gaszenie i konsolę | ręczny FEED zostaje |
| 82 | **POZIOMY I POWIETRZE** — medbay **co 2 poziomy** (3 lv zaraza, 5 lv wirus, `maxLevel` 8), tlen jako budżet produkcji | oddech: załogant 0.04, kot 0.02, szczur 0.01, **pająk 0** |
| 83 | kot na rozkaz | rozkaz trzyma, dopóki nie pojawi się szczur albo ranny |
| 84 | **U10A — szkodniki do wentylacji** | w większości kasowanie kodu; kanał gotowy |
| 85 | **U11 — ogień i powietrze w kanale** | potrzebuje 73 i 84 |
| 86 | **U10B — ekonomia szczurów** | osobno, żeby dało się wyważyć |
...
### V2. Poziomy medbayu bramkują CHOROBY (update82)

**CO DRUGI POZIOM** (korekta gracza 24.09 — było 1/2/3):
...
### V3. Tlen jako budżet produkcji (update82)
...
| kot | **0.02** |
| szczur | **0.01** |
| **pająk** | **0 — nie oddycha**, zrobione w update79 (`SUIT_AIR.NO_LUNGS`) |
...
## USTALENIA Z 24.09.2026 (II) — CO WIDAĆ NA EKRANIE (update80)

Druga porcja zgłoszeń z tej samej rozmowy. Nic z tego nie jest zrobione.

### W1. Pasek znaczników ma mówić WSZYSTKO

Dziś wiersz załoganta pokazuje: chorobę, głód, przy którym module stoi, a od
update78 czy krwawi i czym ma zajęte ręce. Brakuje — **naprawia, gasi pożar,
walczy, brakuje mu powietrza**. Słowa gracza: *„wtedy gracz spojzy na
zaloganta na liscie i wie wszytko co sie dzieje w danym momecie"*.

### W2. Ikony są za małe

`MARK_STEP = 12`, font `bold 10px`. Do powiększenia.

### W3. JEDEN zestaw ikon, nie dwa

To jest właściwa robota tej pozycji, reszta jest przy niej tania.

| gdzie | czym jest dziś |
|---|---|
| nad załogantem na pokładzie | **rysowane ręcznie kształty** w `crew.js` — menażka (elipsa + dwie kreski), butla (prostokąt + wypełnienie) |
| w wierszu na liście | **znaki tekstowe** w `renderer.js` — `▼`, `☣`, `✚`, `◓` |

**To są dwie ikony tej samej rzeczy, w dwóch plikach.** Gracz poprosił wprost:
*„uzyj tych samych co sa nad zalogantami w statku, ta sama ikonka glodu i brak
powietrza co jest"*. Zostaje jeden zestaw, rysowany jedną funkcją, wołany
z obu miejsc. Dopóki są dwa, każda zmiana ikony to zmiana w dwóch plikach —
i pierwsza, o której ktoś zapomni, rozjedzie je na stałe.

### W4. Opatrunki i jedzenie na górnym pasku

Tak samo jak rakiety i He2. Dziś trzeba wejść do ładowni, żeby sprawdzić, czy
jest czym opatrzyć rannego. Czytane **prosto z ładowni** (jak He-3 od
update58), nie z drugiego licznika.

### W5. Pociski wychodzą z działek

Dziś **każdy** strzał wylatuje z `pb.x + pb.w + 10, pb.y + pb.h/2` — z rogu
prostokąta otaczającego CAŁY kadłub, w połowie wysokości. Wszystko jedno,
w której komorze stoi działo.

A działko jest **rysowane** gdzie indziej: `_drawWeaponMounts` układa je wzdłuż
GÓRNEJ krawędzi (`b.y - 42`) i trzyma pozycję w zmiennej lokalnej pętli
rysującej. **Pozycja rysunku i pozycja wystrzału to dwie osobne liczby, które
nigdy się nie spotkały.**

Dobra wiadomość: `combat.js` **już liczy** `gunRoom` dwie linijki dalej (po to,
żeby przypisać XP obsłudze) i wyrzuca go. Zostaje jedno źródło pozycji
mountu, czytane i przez rysunek, i przez wystrzał.

Osobno: **wiązki** (`drawBeam`) w ogóle ignorują punkt startu i rysują się jako
poziomy przejazd po CELU. To druga decyzja, do podjęcia przy tej samej pozycji.
