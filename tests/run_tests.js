...
(function testMemorial() {
  ...
  /* AND THAT SOMEBODY CARRIED HIM BACK. Since update74 that is the
     only kind of man with a stone, so the line no longer asks — but it
     still has to be SAID, or the hill and the NOT RECOVERED panel
     stop telling the player which of the two he is looking at. */
  /* EXACTLY, not by regex: the panel HEADING says "N brought home and
     buried. Hover a marker…" on every draw, so /brought home/ over the
     whole capture passed whether or not the card drew its own line.
     The card's line is that sentence and nothing else. */
  ok(card.some(d => d.t === 'brought home and buried'),
     'and says he was carried home');
...
(function testBurialKarma() {
     ...
     learns here is that he came home. */
  {
       ...
       the service record is tested. */
    {
      ...
      sb.Input.mouse.x = -100; sb.Input.mouse.y = -100;
      // Exact, not a regex: the heading above the hill contains the
      // same words on every draw. See section 85.
      ok(card.some(t => t === 'brought home and buried'),
