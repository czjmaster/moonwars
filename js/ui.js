...
const UI = (() => {
  ...
  /* ONE REGISTER FOR WHAT A SICKNESS LOOKS LIKE (update69).
     `Renderer.DISEASE_COL` decides; this asks it. Three screens had the
     plague's green written out by hand for the spider virus, so the
     deadly one was drawn as the harmless one in the HUD, in the station
     panel and in the barracks. */
  const _virusCol = () =>
    (typeof Renderer !== 'undefined' && Renderer.DISEASE_COL?.virus) || '#d9463c';
  ...
  function _renderStationTab(tab) {
    ...
    switch (tab) {
      ...
      case 'repair': {
        ...
        /* RED, like everywhere else the virus is drawn (update69) —
           this line used the plague's green and said VIRUS. */
        if (sick) line(st, 'INFECTED', `${sick} carrying the virus`, _virusCol());
        ...
        // ── QUARANTINE WARD — research posts only ──
        {
          ...
          if (carriers.length) {
            ...
            const t = document.createElement('div');
            t.style.cssText = `color:${_virusCol()};font-size:13px;font-weight:bold`;
            t.textContent = '☣ QUARANTINE WARD';
            d.appendChild(t);
            line(d, 'carriers', carriers.map(c => c.name).join(', '), _virusCol());
            if (!isSci) {
              line(d, '', 'No ward here. Only a RESEARCH POST can treat the void-spider '
                        + 'virus — the clinic cannot touch it.', '#ff5566');
              line(d, '', 'Untreated, a carrier turns before the contract is out and leaves '
        ...
        [
          ...
          /* RATIONS (update66) — one card per kind the port carries.
             Built from the stock rather than a fixed list of four, so a
             port that has no greens shows no greens instead of a row
             saying "sold out" for something it never stocked. */
          ...Object.entries(s.stock.rations || {}).map(([key, qty]) => {
            const def = CARGO_ITEMS[key];
            return {
              key, title: def.label.toUpperCase(), stock: qty,
              unit: s.rationCost(key),
              have: _stationShip?.cargo ? _stationShip.cargo.countOf('food') : 0,
              col: def.col,
              note: `${def.hunger} hunger · ${def.w * def.h} cell`
                  + `${def.w * def.h > 1 ? 's' : ''}`
                  + (def.meat === false ? ' · no meat — every mouth aboard eats it'
                                        : ''),
              buy: (n) => s.buyRations(key, n, run, _stationShip),
            };
          }),
        ].forEach(item => {
          const d = card(right);
      ...
      case 'weapons': {
        ...
        ship.weaponRooms.forEach((room, slot) => {
          ...
          if (w) {
            ...
            // "→ HOLD", not "→ CARGO": CARGO was the rack's name for
            // itself, and the screenshot for this package still had the
            // button pointing at a place that no longer exists.
            btn(d, 'UNINSTALL → HOLD', true, () => {
              const r = s.uninstallWeapon(ship, slot);
              notify(r.message, r.ok ? 'good' : 'warn');
              _renderStation();
            }, '#ffb020');
          } else {
            sub(d, `Bay level ${lvl}. Fit a gun out of the hold, or buy one.`);
            /* ONE LIST, NOT TWO (update75). This drew the loose rack AND
               the boxed crates, one FIT button per list, so the same gun
               could appear twice under two different verbs. There is
               only the hold now. */
            const boxed = (ship.cargo?.items ?? [])
              .filter(it => it.def.kind === 'weapon' && it.meta);
            boxed.forEach(it => {
              const wdef = WEAPON_DEFS[it.meta] || {};
              btn(d, `FIT ${wdef.label ?? it.meta}  (${it.w}x${it.h} crate)`, true, () => {
                const r = s.installFromCargo(ship, it, slot);
                notify(r.message, r.ok ? 'good' : 'warn');
                _renderStation();
              }, '#1aff8c');
            });
            if (!boxed.length) sub(d, 'No gun crates in the hold.', '#4a6080');
          }
        });

        /* ── GUNS IN THE HOLD ────────────────────────────────
         *
         * One section, one truth. Before update75 this screen had a
         * CARGO RACK list and a boxed-crate list side by side, with
         * BOX INTO HOLD and UNBOX ONTO THE RACK to shuffle a gun
         * between them — two homes for one object, and a player who
         * had to know which one he was looking at. The rack is gone;
         * every spare gun is a crate that occupies real cells. */
        const crates = (ship.cargo?.items ?? [])
          .filter(it => it.def.kind === 'weapon' && it.meta);
        ...
        rackHead.textContent = `GUNS IN THE HOLD (${crates.length})`
          + (ship.cargo ? `  ·  ${ship.cargo.usedCells()}/${ship.cargo.capacity} cells used` : '');
        left.appendChild(rackHead);

        if (!crates.length) {
          const d = card(left);
          sub(d, 'No spare guns aboard. A gun is either bolted on or boxed in the hold — '
               + 'and a crate takes up real cells, so a gun you keep is a gun you are '
               + 'carrying instead of cargo.', '#4a6080');
        }
        ...
        crates.forEach(it => {
          ...
          btn(d, freeMount !== -1 ? `FIT → BAY ${freeMount + 1}` : 'NO FREE MOUNT',
              freeMount !== -1, () => {
                const r = s.installFromCargo(ship, it, freeMount);
                notify(r.message, r.ok ? 'good' : 'warn');
                _renderStation();
              }, '#1aff8c');
          btn(d, `SELL — ${Math.floor((def.cost ?? 20) * 0.5)} CC`, true, () => {
            const r = s.sellCargoWeapon(ship, run, it);
            notify(r.message, r.ok ? 'good' : 'warn');
            _renderStation();
          }, '#ff7c20');
          if (freeMount === -1) {
            sub(d, 'Every bay is full. Free one above, or sell this.', '#7a90a8');
          }
        });
        ...
        const right = column('STATION STOCK',
          freeBay !== -1
            ...
            : 'All bays are full — a purchase is boxed into the hold, and needs the room.');
        ...
        s.stock.weapons.forEach((item, i) => {
          ...
          /* WHAT IT COSTS IN SPACE, not just in CC (update75). With no
             free bay the gun arrives as a crate, and the player has to
             be able to see how big it is BEFORE he pays. */
          if (freeBay === -1 && typeof cargoCrateForWeapon === 'function'
              && typeof CargoItem !== 'undefined') {
            const probe = new CargoItem(cargoCrateForWeapon(item.key));
            sub(d, `Arrives boxed: ${probe.w}x${probe.h} cells in the hold.`, '#4db8ff');
          }
          if (item.sold) {
