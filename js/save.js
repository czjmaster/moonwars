...
const Save = (() => {
  ...
  /* ── EVERY POSTER HAS AN ADDRESS (update70) ────────────────
   *
   * A name on the board used to say only "he is out there", and the
   * map would seat ANY of them on ANY contract — so the board could
   * not tell the player where to look and four posters were four
   * identical lottery tickets. The player asked for the opposite:
   * "nie ze sa tylko w jednym kontrakcie ale w roznych, np pirat a
   * jest w 1, pirat bc w 2".
   *
   * The contract is a FIELD on the poster, and the list of contracts
   * is read from `MISSIONS` at call time rather than copied here —
   * base.js loads after this file, and a second list of contract keys
   * is a list that would be wrong the day a fourth job is added.
   */
  function _missionKeys() {
    return (typeof MISSIONS !== 'undefined' && MISSIONS)
      ? Object.keys(MISSIONS) : ['patrol'];
  }

  /** The contract carrying the fewest names takes the next one. Ties
   *  break at random, so a fresh board is not the same board twice. */
  function _spreadMission() {
    const keys  = _missionKeys();
    const count = {};
    keys.forEach(k => { count[k] = 0; });
    (_data?.wanted ?? []).forEach(w => {
      if (count[w.mission] != null) count[w.mission]++;
    });
    const min  = Math.min(...keys.map(k => count[k]));
  ...
  /** An older save's board has names with no contract on them. Give
   *  them one — spread the same way a new name is — and only once. */
  function _addressOldPosters() {
    if (!Array.isArray(_data?.wanted)) return false;
    let changed = false;
    _data.wanted.forEach(w => {
      if (w && !w.mission) { w.mission = _spreadMission(); changed = true; }
    });
    return changed;
  }

  /** Everyone on the board who can be met on THIS contract. */
  function wantedFor(missionId) {
    if (!missionId) return wanted();
    return wanted().filter(w => w.mission === missionId);
  }
  ...
  function load() {
    ...
    _seedWanted();
    /* …and a board from before update70 gets an address on every name.
       Written straight back, so the contract a poster is seen in does
       not change between one load and the next. */
    if (_addressOldPosters()) save();
