...
/* CONTINUE LIVES IN THE BASE NOW (update57). The title screen asked
   the player to decide between "enter the base" and "carry on the
   contract" before he could see the state of either — and a contract
   in the air is a fact ABOUT the base: the hangar is short a hull and
   the barracks is short a crew until it comes home. Two doors, and the
   base is the room they both open into. */
const MENU_ITEMS = ['ENTER BASE','OPTIONS'];
  ...
  function _updateMenu() {
    ...
    if (Input.mouse.leftPressed && _menuHover !== null) {
      ...
      try {
        ...
        if (_menuHover === 1) { STATE = 'options'; _beginFade(); }
      } catch (e) {
  ...
  // ── MAP ───────────────────────────────────────────────────
  /**
   * TAB, wherever the player presses it (update74).
   *
   * It used to mean two different things in two places: in a fight it
   * walked the crew list, and on the MAP it was a second key for M and
   ...
   * came back as a bug report. TAB is the crew key now, everywhere.
   * M and the on-screen button are the view.
   *
   * Lifted into a function rather than copied into the second place,
   * because two copies of a selection rule drift the first time one of
   * them learns about a new kind of crewman.
   */
  function _cycleCrew() {
  ...
  function _updateMap(dt) {
    ...
    // TAB is the CREW key on every screen (update74) — M and the button
    // are the view. They used to share TAB, so the same press did two
    // different things depending on what was up.
  ...
  /* ── THE TWO BUTTONS CLEAR THE OBJECTIVE LINE (update74) ──
   *
   * They sat at y 42 and 70, and the run-goal line sits at y 40 with a
   * height of 17 — so SHOW MAP was drawn straight through it and the
   * player read "OBJ Cut them dow…" with a button on top of the rest.
   *
   * Both moved down by the height of that line. The gap is measured
   * from `Renderer.runGoalsRect()` rather than typed in here, so there
   * is one description of where the line is and these buttons are
   * placed against it instead of beside it. FIXED, not conditional:
   * a button that moves depending on whether this contract carries
   * objectives is a button the hand has to look for.
   */
  const _TOP_BTN_W = 150, _TOP_BTN_H = 24, _TOP_BTN_GAP = 6;
  function _topBtnY() {
    // `runGoalsBox`, not `runGoalsRect`: where the line LIVES, not
    // whether it has anything on it today. Asking the second question
    // here is what would make the buttons move between contracts.
    const box = Renderer.runGoalsBox ? Renderer.runGoalsBox() : { y: 40, h: 17 };
    return box.y + box.h + _TOP_BTN_GAP;
  }

  /* SIDE BY SIDE, not stacked (update74).
   *
   * Moving them below the objective line was half the fix and the
   * screenshot showed the other half: stacked, CARGO landed at y 89
   * and drove straight through the green "CHOOSE YOUR STARTING LANE"
   * banner — the very first thing a player sees in a run. The strip
   * between the resources row and the map panel is 40..115, which is
   * not tall enough for two rows of buttons and a banner.
   *
  ...
  function _updateCombat(dt) {
    ...
    if (!_derelictOffered && !BossManager.isActive && _enemyShip &&
      ...
      /* ── THEIR DECKS ARE CLEARED. IS ANYONE LEFT TO STRIKE? (update62)
       *
       * This branch already owned the moment "the enemy crew is wiped
       * out" — it just assumed there was nobody left to talk to. There
       * often is: the enemy COMMANDER is a record, not a body, so he
       * survives his crew, and clearing their decks leaves him alone
       * with your boarding party. That is the moment the karma design
       * asks for, and it belongs HERE rather than in a second trigger
       * of its own — one situation, one place that decides it.
       *
       * The hulk offer is untouched for a ship that never had one.
       */
      const foeCap = (typeof Commander !== 'undefined') ? Commander.enemy() : null;
      const noQuarter = CombatManager.noQuarter?.() ?? false;
      if (foeCap && noQuarter && !_noQuarterSaid) {
        _noQuarterSaid = true;
  ...
  function _resolveEvent(idx) {
    ...
    if (result.dockWreck) {
      ...
      _beginDocking(sector, {
        seconds: result.seconds ?? Ship.LOOT_SECONDS,
        rich: !!result.rich,
  ...
  /* Set by `_startWreckBoarding` before anything reads it — the value
     here is never the one used. Seeded from the constant anyway so a
     future reader that arrives early gets a sane clock rather than a
     zero, which would read as "out of time" the moment it was shown. */
  let _wreckSecs   = Ship.LOOT_SECONDS;
  ...
  function _updateDocking(dt) {
    ...
    _startWreckBoarding(pend.sector, {
      ...pend,
      seconds: Math.max(Ship.LOOT_SECONDS_MIN, (pend.seconds ?? Ship.LOOT_SECONDS) + out.bonusSeconds),
    });
  }
  ...
  function _openWreckLoot(sector, opts = {}) {
    ...
    let seconds = opts.seconds ?? Ship.LOOT_SECONDS;
    if (Math.random() < 0.18) {
  ...
  function _updateBase(dt) {
    ...
    /* THE CONTRACT STILL IN THE AIR (update57) — resumed from the base,
       which is the only screen that can tell you what is riding on it. */
    if (action === 'continue') _continueRun();
