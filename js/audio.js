...
const Audio = (() => {
  ...
  function _scheduleBeat() {
    ...
    const interval = _musicMode === 'boss' ? 300 : 500;

    _tone({
      freq: note, freqEnd: note * 0.98,
      type: 'triangle', duration: 0.4,
      gain: 0.25, attack: 0.01, release: 0.3,
    ...
    if (_musicBeat % 4 === 0) {
      _tone({
        freq: note / 2, type: 'sine', duration: 0.6,
        gain: 0.3, attack: 0.02, release: 0.5,
        destination: musicGain,
      });
    }

    _musicBeat++;
    _musicTimer = setTimeout(_scheduleBeat, interval);
  }
