...
class Combat {
  ...
  _updateAI(dt) {
    ...
    if (!this._aiTargetRoom || Math.random() < 0.005) {
      if (Math.random() < this._ai.targetRandom) {
        ...
        // Prefer shields, then weapons
        this._aiTargetRoom =
          player.rooms.find(r => r.type === 'shields') ||
    ...
    enemy.weapons.forEach((w, i) => {
      ...
      const fromX = enemy.worldX;
      const fromY = enemy.worldY + enemy.spriteH / 2;
    ...
    // Enemy crew AI — send to damaged rooms
    enemy.systems.forEach(sys => {
      if (sys.damage > 0 && !sys.isFullyRepaired()) {
        const idleCrew = enemy.crew.find(c => c.task === TASK.IDLE);
        if (idleCrew) idleCrew.assignTask(TASK.REPAIR, sys.roomId);
      }
    });
    enemy.fires.fires.forEach(fire => {
      if (!fire.out) {
        const idleCrew = enemy.crew.find(c => c.task === TASK.IDLE);
        if (idleCrew) idleCrew.assignTask(TASK.FIRE, fire);
      }
    });
  }
  ...
  playerFire(weapon) {
    ...
    const enemy  = this.enemyShip;
    const target = Utils.pick(enemy.rooms);

    const fromX  = this.playerShip.worldX + this.playerShip.spriteW;
    const fromY  = this.playerShip.worldY + this.playerShip.spriteH / 2;
