...
class Ship {
  ...
  /** Cells aboard: one per WORKING level of the brig, so a shot-out
   *  brig is smaller before it is empty. No brig, no cells. */
  brigCapacity() {
    const b = this.getSystem('brig');
    return b ? Math.max(0, b.workingLevels) : 0;
  }

  /** Cells with nobody in them. */
  freeCells() { return Math.max(0, this.brigCapacity() - this.prisoners.length); }
  ...
  prisonerTick(dt) {
    ...
    const held = !!brig && !brig.isDisabled();
    if (held) {
      this.prisoners.forEach(p => { p.escapeT = 0; p.warned = false; });
      return;
    }
    const gone = [];
    this.prisoners.forEach(p => {
      ...
      if (!p.warned && p.escapeT >= Ship.ESCAPE_SECONDS * 0.4) {
        p.warned = true;
        if (this.isPlayer && typeof UI !== 'undefined') {
          UI.notify(`${p.name} is working the cell door — get the brig powered!`, 'alert');
        }
      }
